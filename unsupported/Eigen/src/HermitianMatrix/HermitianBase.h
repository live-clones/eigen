...
class HermitianBase : public EigenBase<Derived> {
 public:
  ...
  typedef typename internal::conditional<bool(internal::traits<Derived>::Flags& LvalueBit), const Scalar&,
                                         typename internal::conditional<internal::is_arithmetic<Scalar>::value, Scalar,
                                                                        const Scalar>::type>::type CoeffReturnType;
#endif
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Derived& operator=(const HermitianBase<OtherDerived>& other) {
    internal::call_assignment(derived(), other.derived());
    return derived();
  }

  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Derived& _set(const HermitianBase<OtherDerived>& other) {
    internal::call_assignment(derived(), other.derived());
    return derived();
  }
   ...
   *  column */
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression> operator()(
      Index row, Index col) {
  ...
  EIGEN_DEVICE_FUNC
  const NestedExpression& nestedExpression() const { return derived().nestedExpression(); }

  /** \returns a reference to the nested expression */
  EIGEN_DEVICE_FUNC
  NestedExpression& nestedExpression() { return derived().nestedExpression(); }

  /** \returns the number of columns. \sa rows(), ColsAtCompileTime*/
  EIGEN_DEVICE_FUNC
  Index cols() { return derived().cols(); }
  ...
  EIGEN_DEVICE_FUNC
  Index rows() { return derived().rows(); }
  ...
  template <typename OtherDerived>
  EIGEN_STRONG_INLINE const CwiseBinaryOp<internal::scalar_sum_op<typename internal::traits<Derived>::Scalar,
  ...
  // hermitian * hermitian
  template <typename OtherDerived>
  EIGEN_STRONG_INLINE const Product<Derived, OtherDerived, LazyProduct> operator*(
      const HermitianBase<OtherDerived>& rhs) const {
