...
struct TensorEvaluator<const TensorTraceOp<Dims, ArgType>, Device> {
 ...
 protected:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr Index firstInput(Index index) const { return firstInputImpl(index); }

  template <int ND = NumOutputDims>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::enable_if_t<ND == 0, Index> firstInputImpl(
      Index /*index*/) const {
    return 0;
  }

  template <int ND = NumOutputDims>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::enable_if_t<(ND > 0), Index> firstInputImpl(Index index) const {
    Index startInput = 0;
    EIGEN_IF_CONSTEXPR (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
