...
struct DenseFunctor {
  ...
  constexpr DenseFunctor() = default;
...
class LevenbergMarquardt : internal::no_assignment_operator {
 ...
 public:
  ...
  static constexpr LevenbergMarquardtSpace::Status lmdif1(
      FunctorType &functor, FVectorType &x, Index *nfev, const Scalar tol = numext::sqrt(NumTraits<Scalar>::epsilon()));
  ...
  constexpr Index iterations() const { return m_iter; }

  /** \returns the number of functions evaluation */
  constexpr Index nfev() const { return m_nfev; }

  /** \returns the number of jacobian evaluation */
  constexpr Index njev() const { return m_njev; }

  /** \returns the norm of current vector function */
  constexpr RealScalar fnorm() const { return m_fnorm; }

  /** \returns the norm of the gradient of the error */
  constexpr RealScalar gnorm() const { return m_gnorm; }

  /** \returns the LevenbergMarquardt parameter */
  constexpr RealScalar lm_param(void) const { return m_par; }
   ...
   */
  constexpr PermutationType permutation() const { return m_permutation; }
