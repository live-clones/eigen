...
if [[ "${EIGEN_CI_SKIP_APT}" != "true" ]]; then
  ...
  apt-get install -y --no-install-recommends ninja-build cmake git xsltproc ca-certificates > /dev/null
...
elif [[ "${CI_JOB_STAGE}" == "build" && ! -e /etc/ssl/certs/ca-certificates.crt ]]; then
  ...
  # Test jobs only run the built executables and do not need this dependency.
  apt-get update -y > /dev/null
