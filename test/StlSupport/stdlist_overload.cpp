...
TEST(StdlistOverloadTest, Basic) {
  // some non vectorizable fixed sizes
  check_stdlist_matrix(Vector2f());
  check_stdlist_matrix(Matrix3f());
  check_stdlist_matrix(Matrix3d());

  // some vectorizable fixed sizes
  check_stdlist_matrix(Matrix2f());
  check_stdlist_matrix(Vector4f());
  check_stdlist_matrix(Matrix4f());
  check_stdlist_matrix(Matrix4d());

  // some dynamic sizes
  check_stdlist_matrix(MatrixXd(1, 1));
  check_stdlist_matrix(VectorXd(20));
  check_stdlist_matrix(RowVectorXf(20));
  check_stdlist_matrix(MatrixXcf(10, 10));

  // some Transform
  check_stdlist_transform(Affine2f());  // does not need the specialization (2+1)^2 = 9
  check_stdlist_transform(Affine3f());
  check_stdlist_transform(Affine3d());

  // some Quaternion
  check_stdlist_quaternion(Quaternionf());
  check_stdlist_quaternion(Quaterniond());
}
