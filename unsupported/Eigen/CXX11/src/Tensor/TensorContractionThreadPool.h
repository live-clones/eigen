   ...
   private:
    ...
    void signal_kernel(Index m, Index n, Index k, bool sync, bool use_thread_local) {
      ...
      if (sync) {
        ...
        eigen_assert(!use_thread_local);
        device_.enqueueNoNotification([this, m, n, k, use_thread_local]() { kernel(m, n, k, use_thread_local); });
      }
    }
    ...
    void enqueue_packing_helper(Index start, Index end, Index k, bool rhs) {
      if (end - start == 1) {
        ...
        while (end - start > 1) {
          ...
          device_.enqueueNoNotification([this, mid, end, k, rhs]() { enqueue_packing_helper(mid, end, k, rhs); });
          end = mid;
        }
        ...
        if (pack_async) {
          device_.enqueueNoNotification([this, start, end, k, rhs]() { enqueue_packing_helper(start, end, k, rhs); });
        } else {
