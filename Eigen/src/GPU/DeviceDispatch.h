...
// Dispatch functions that map gpu::DeviceMatrix expressions to NVIDIA library calls.
//
...
void dispatch_gemm(
    gpu::Context& ctx, gpu::DeviceMatrix<typename device_expr_traits<Lhs>::scalar_type>& dst,
    const gpu::GemmExpr<Lhs, Rhs>& expr, typename device_expr_traits<Lhs>::scalar_type beta_val,
  ...
  const gpu::DeviceMatrix<Scalar>& A = traits_lhs::matrix(expr.lhs());
  const gpu::DeviceMatrix<Scalar>& B = traits_rhs::matrix(expr.rhs());
  ...
  eigen_assert(k == rhs_k && "gpu::DeviceMatrix GEMM dimension mismatch");
...
void dispatch_llt_solve(gpu::Context& ctx, gpu::DeviceMatrix<Scalar>& dst, const gpu::LltSolveExpr<Scalar, UpLo>& expr) {
  const gpu::DeviceMatrix<Scalar>& A = expr.matrix();
  const gpu::DeviceMatrix<Scalar>& B = expr.rhs();
...
void dispatch_lu_solve(gpu::Context& ctx, gpu::DeviceMatrix<Scalar>& dst, const gpu::LuSolveExpr<Scalar>& expr) {
  const gpu::DeviceMatrix<Scalar>& A = expr.matrix();
  const gpu::DeviceMatrix<Scalar>& B = expr.rhs();
...
template <typename Scalar, int UpLo>
void dispatch_trsm(gpu::Context& ctx, gpu::DeviceMatrix<Scalar>& dst, const gpu::TrsmExpr<Scalar, UpLo>& expr) {
  const gpu::DeviceMatrix<Scalar>& A = expr.matrix();
  const gpu::DeviceMatrix<Scalar>& B = expr.rhs();
...
template <typename Scalar, int UpLo>
void dispatch_symm(gpu::Context& ctx, gpu::DeviceMatrix<Scalar>& dst, const gpu::SymmExpr<Scalar, UpLo>& expr) {
  const gpu::DeviceMatrix<Scalar>& A = expr.matrix();
  const gpu::DeviceMatrix<Scalar>& B = expr.rhs();
...
template <typename Scalar, int UpLo>
void dispatch_syrk(gpu::Context& ctx, gpu::DeviceMatrix<Scalar>& dst, const gpu::SyrkExpr<Scalar, UpLo>& expr,
 ...
 public:
  ...
  DeviceMatrix<Scalar>& operator=(const Expr&) {
    static_assert(sizeof(Expr) == 0,
                  "gpu::DeviceMatrix expression not supported: no cuBLAS/cuSOLVER mapping. "
