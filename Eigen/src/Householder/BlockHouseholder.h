...
void make_block_householder_triangular_factor(TriangularFactorType& triFactor, const VectorsType& vectors,
  ...
  for (Index i = nbVecs - 1; i >= 0; --i) {
    ...
    if (rt > 0) {
      ...
      // Note: Eigen does not support in-place triangular products (the result aliases the input),
      // so we use the manual loop below instead of:
      // triFactor.row(i).tail(rt).noalias() = triFactor.row(i).tail(rt) *
      //     triFactor.bottomRightCorner(rt,rt).template triangularView<Upper>();
...
void apply_block_householder_on_the_left(MatrixType& mat, const VectorsType& vectors, const CoeffsType& hCoeffs,
  ...
  Matrix<typename MatrixType::Scalar, VectorsType::ColsAtCompileTime, MatrixType::ColsAtCompileTime,
      ...
      tmp = V.adjoint() * mat;
  // Note: .noalias() cannot be used here because tmp aliases both sides of the triangular product.
  if (forward)
...
void apply_block_householder_on_the_right(MatrixType& mat, const VectorsType& vectors, const CoeffsType& hCoeffs,
  ...
  // Note: .noalias() cannot be used here because tmp aliases both sides of the triangular product.
  if (forward)
    tmp = tmp * T.template triangularView<Upper>();
  else
    tmp = tmp * T.template triangularView<Upper>().adjoint();
