...
bool bicgstab(const MatrixType& mat, const Rhs& rhs, Dest& x, const Preconditioner& precond, Index& iters,
  ...
  while (r_sqnorm > tol2 && i < maxIters) {
    Scalar rho_old = rho;
    rho = r0.dot(r);
    if (Eigen::numext::abs2(rho) < eps2 * r0_sqnorm * r_sqnorm) {
    ...
    if (Eigen::numext::abs2(theta) < eps2 * r0_sqnorm * v.squaredNorm()) {
      r = rhs - mat * x;
      r0.setRandom();
    ...
    x += alpha * y + w * z;
    r = s - w * t;
    r_sqnorm = r.squaredNorm();
    ++i;
  }

  tol_error = Eigen::numext::sqrt(r_sqnorm / rhs_sqnorm);
  iters = i;
  return true;
}
