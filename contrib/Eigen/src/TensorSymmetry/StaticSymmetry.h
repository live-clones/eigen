...
static constexpr std::vector<Index> tensor_static_symgroup_index_permute(const std::vector<Index>& idx,
                                                                         std::integer_sequence<int, ii...>) {
...
struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>> {
  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index, std::size_t NumIndices,
            typename... Args>
  static constexpr RV run(const std::array<Index, NumIndices>& idx, RV initial, Args&&... args) {
  ...
  static constexpr RV run(const std::vector<Index>& idx, RV initial, Args&&... args) {
...
struct tensor_static_symgroup_do_apply<internal::type_list<>> {
  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index, std::size_t NumIndices,
            typename... Args>
  static constexpr RV run(const std::array<Index, NumIndices>&, RV initial, Args&&...) {
    // do nothing
    return initial;
  }

  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index, typename... Args>
  static constexpr RV run(const std::vector<Index>&, RV initial, Args&&...) {
    // do nothing
    return initial;
  }
};
...
class StaticSGroup {
 ...
 public:
  ...
  static constexpr RV apply(const std::array<Index, N>& idx, RV initial, Args&&... args) {
    return internal::tensor_static_symgroup_do_apply<ge>::template run<Op, RV, NumIndices>(idx, initial, args...);
  }

  template <typename Op, typename RV, typename Index, typename... Args>
  static constexpr RV apply(const std::vector<Index>& idx, RV initial, Args&&... args) {
  ...
  constexpr internal::tensor_symmetry_value_setter<Tensor_, StaticSGroup<Gen...>> operator()(
