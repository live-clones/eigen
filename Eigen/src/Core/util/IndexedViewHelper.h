...
constexpr IvcType<Indices, SizeAtCompileTime> CreateIndexSequence(size_t size, const Indices& indices) {
...
struct IndexedViewSelector<Derived, RowIndices, ColIndices,
  ...
  static constexpr ReturnType run(Derived& derived, const RowIndices& rowIndices, const ColIndices& colIndices) {
  ...
  static constexpr ConstReturnType run(const Derived& derived, const RowIndices& rowIndices,
...
struct VectorIndexedViewSelector<
  ...
  static constexpr RowMajorReturnType run(Derived& derived, const Indices& indices) {
    return RowMajorReturnType(derived, ZeroIndex(0),
                              CreateIndexSequence<Derived::ColsAtCompileTime>(derived.cols(), indices));
  }
  template <bool UseRowMajor = IsRowMajor, std::enable_if_t<UseRowMajor, bool> = true>
  static constexpr ConstRowMajorReturnType run(const Derived& derived, const Indices& indices) {
    return ConstRowMajorReturnType(derived, ZeroIndex(0),
                                   CreateIndexSequence<Derived::ColsAtCompileTime>(derived.cols(), indices));
  }
  template <bool UseRowMajor = IsRowMajor, std::enable_if_t<!UseRowMajor, bool> = true>
  static constexpr ColMajorReturnType run(Derived& derived, const Indices& indices) {
    return ColMajorReturnType(derived, CreateIndexSequence<Derived::RowsAtCompileTime>(derived.rows(), indices),
                              ZeroIndex(0));
  }
  template <bool UseRowMajor = IsRowMajor, std::enable_if_t<!UseRowMajor, bool> = true>
  static constexpr ConstColMajorReturnType run(const Derived& derived, const Indices& indices) {
...
struct VectorIndexedViewSelector<
  ...
  static constexpr ReturnType run(Derived& derived, const Indices& indices) {
  ...
  static constexpr ConstReturnType run(const Derived& derived, const Indices& indices) {
    auto actualIndices = CreateIndexSequence<Derived::SizeAtCompileTime>(derived.size(), indices);
