...
struct DSizes : array<DenseIndex, NumDims> {
  ...
  template <typename OtherIndex,
            std::enable_if_t<
                ...
                int> = 0>
  EIGEN_DEVICE_FUNC constexpr explicit DSizes(const array<OtherIndex, NumDims>& other) {
    for (int i = 0; i < NumDims; ++i) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr DSizes(DenseIndex firstDimension, DenseIndex secondDimension,
