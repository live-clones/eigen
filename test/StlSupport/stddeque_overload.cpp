...
EIGEN_DECLARE_TEST(stddeque_overload) {
  ...
  check_stddeque_matrix(Matrix3d());

  // some vectorizable fixed sizes
  check_stddeque_matrix(Matrix2f());
  check_stddeque_matrix(Vector4f());
  check_stddeque_matrix(Matrix4f());
  check_stddeque_matrix(Matrix4d());

  // some dynamic sizes
  check_stddeque_matrix(MatrixXd(1, 1));
  check_stddeque_matrix(VectorXd(20));
  check_stddeque_matrix(RowVectorXf(20));
  check_stddeque_matrix(MatrixXcf(10, 10));

  // some Transform
  check_stddeque_transform(Affine2f());  // does not need the specialization (2+1)^2 = 9
  check_stddeque_transform(Affine3f());
  check_stddeque_transform(Affine3d());

  // some Quaternion
  check_stddeque_quaternion(Quaternionf());
  check_stddeque_quaternion(Quaterniond());
}
