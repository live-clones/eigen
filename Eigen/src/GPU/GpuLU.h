...
class LU {
 public:
  ...
  LU& compute(const EigenBase<InputType>& A) {
    ...
    allocate_lu_storage();
    EIGEN_CUDA_RUNTIME_CHECK(cudaMemcpyAsync(d_lu_.ptr, mat.data(), matrixBytes(), cudaMemcpyHostToDevice, ctx_.stream_));
  ...
  LU& compute(const DeviceMatrix<Scalar>& d_A) {
    ...
    allocate_lu_storage();
    EIGEN_CUDA_RUNTIME_CHECK(cudaMemcpyAsync(d_lu_.ptr, d_A.data(), matrixBytes(), cudaMemcpyDeviceToDevice, ctx_.stream_));
 ...
 private:
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(
        cusolverDnXgetrf(ctx_.cusolver_, ctx_.params_.p, static_cast<int64_t>(n_), static_cast<int64_t>(n_), dtype,
                         d_lu_.ptr, lda_, static_cast<int64_t*>(d_ipiv_.ptr), dtype, ctx_.scratch_workspace(),
                         dev_ws_bytes, host_ws_bytes > 0 ? ctx_.h_workspace_.data() : nullptr, host_ws_bytes,
                         ctx_.scratch_info()));
