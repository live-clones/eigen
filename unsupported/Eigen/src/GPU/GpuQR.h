...
class QR {
 public:
  ...
  PlainMatrix solve(const MatrixBase<Rhs>& B) const {
    ...
    EIGEN_CUBLAS_CHECK(internal::cublasXtrsm(ctx_.cublas_, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                                             ...
                                             static_cast<const Scalar*>(d_qr_.get()), static_cast<int>(lda_),
                                             static_cast<Scalar*>(d_B.get()), static_cast<int>(ldb)));
  ...
  DeviceMatrix<Scalar> solve(const DeviceMatrix<Scalar>& d_B) const {
    ...
    EIGEN_CUBLAS_CHECK(internal::cublasXtrsm(ctx_.cublas_, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
                                             CUBLAS_DIAG_NON_UNIT, static_cast<int>(n_), static_cast<int>(nrhs), &alpha,
                                             static_cast<const Scalar*>(d_qr_.get()), static_cast<int>(lda_),
                                             static_cast<Scalar*>(d_work.get()), static_cast<int>(ldb)));
 ...
 private:
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgeqrf_bufferSize(ctx_.cusolver_, ctx_.params_.p, m_,
                                                     n_, dtype, d_qr_.get(), lda_, dtype,
                                                     d_tau_.get(), dtype, &dev_ws, &host_ws));

    ctx_.ensure_scratch(dev_ws);
    ctx_.h_workspace_.resize(host_ws);

    EIGEN_CUSOLVER_CHECK(
        cusolverDnXgeqrf(ctx_.cusolver_, ctx_.params_.p, m_, n_, dtype,
                         d_qr_.get(), lda_, dtype, d_tau_.get(), dtype, ctx_.scratch_workspace(), dev_ws,
  ...
  void apply_QH(void* d_B, int64_t ldb, int64_t nrhs) const {
    ...
    EIGEN_CUSOLVER_CHECK(internal::cusolverDnXormqr_bufferSize(
        ctx_.cusolver_, CUBLAS_SIDE_LEFT, trans, im, in, ik, static_cast<const Scalar*>(d_qr_.get()), ilda,
    ...
    EIGEN_CUSOLVER_CHECK(internal::cusolverDnXormqr(
        ctx_.cusolver_, CUBLAS_SIDE_LEFT, trans, im, in, ik, static_cast<const Scalar*>(d_qr_.get()), ilda,
        static_cast<const Scalar*>(d_tau_.get()), static_cast<Scalar*>(d_B), ildb, static_cast<Scalar*>(d_work.get()),
