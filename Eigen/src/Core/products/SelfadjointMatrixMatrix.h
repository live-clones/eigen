...
struct symm_pack_lhs {
  ...
  using DirectPacker =
      gemm_pack_lhs<Scalar, Index, Mapper, Pack1, packet_traits<Scalar>::size, PacketType, StorageOrder, false, false>;
  using MirroredPacker = gemm_pack_lhs<Scalar, Index, TransposedMapper, Pack1, packet_traits<Scalar>::size, PacketType,
  ...
  void pack_panel(Scalar* blockA, const Mapper& lhs, const TransposedMapper& lhs_t, Index cols, Index i, Index bw,
    ...
    if (cols > i + bw) MirroredPacker()(panel + (i + bw) * bw, lhs_t.getSubMapper(i, i + bw), cols - (i + bw), bw);
    count += bw * numext::maxi(cols, i + bw);
  }
  ...
  void operator()(Scalar* blockA, const Scalar* lhs_, Index lhsStride, Index cols, Index rows) const {
    ...
    constexpr Index HalfPacketSize = unpacket_traits<HalfPacket>::size;
    constexpr Index QuarterPacketSize = unpacket_traits<QuarterPacket>::size;
    ...
    EIGEN_IF_CONSTEXPR (Pack1 >= 3 * PacketSize)
      for (Index i = 0; i < peeled_mc3; i += 3 * PacketSize)
        pack_panel(blockA, lhs, lhs_t, cols, i, 3 * PacketSize, count);

    EIGEN_IF_CONSTEXPR (Pack1 >= 2 * PacketSize)
      for (Index i = peeled_mc3; i < peeled_mc2; i += 2 * PacketSize)
        pack_panel(blockA, lhs, lhs_t, cols, i, 2 * PacketSize, count);

    EIGEN_IF_CONSTEXPR (Pack1 >= 1 * PacketSize)
      for (Index i = peeled_mc2; i < peeled_mc1; i += 1 * PacketSize)
        pack_panel(blockA, lhs, lhs_t, cols, i, 1 * PacketSize, count);

    EIGEN_IF_CONSTEXPR (HasHalf && Pack1 >= HalfPacketSize)
      for (Index i = peeled_mc1; i < peeled_mc_half; i += HalfPacketSize)
        pack_panel(blockA, lhs, lhs_t, cols, i, HalfPacketSize, count);

    EIGEN_IF_CONSTEXPR (HasQuarter && Pack1 >= QuarterPacketSize)
      for (Index i = peeled_mc_half; i < peeled_mc_quarter; i += QuarterPacketSize)
        pack_panel(blockA, lhs, lhs_t, cols, i, QuarterPacketSize, count);

    // do the same with mr==1
    for (Index i = peeled_mc_quarter; i < rows; i++) pack_panel(blockA, lhs, lhs_t, cols, i, 1, count);
  }
};
...
struct symm_pack_rhs {
  ...
  void pack_diagonal_panel(Scalar* blockB, const Mapper& rhs, const TransposedMapper& rhs_t, Index k2, Index end_k,
    ...
    Index h = 0;
    for (Index k = j2; k < j2 + Width; k++) {
      for (Index w = 0; w < h; w++) blockB[count + w] = rhs(k, j2 + w);
      blockB[count + h] = numext::real(rhs(k, k));
      for (Index w = h + 1; w < Width; w++) blockB[count + w] = numext::conj(rhs(j2 + w, k));
      count += Width;
      ++h;
    }
    if (end_k > j2 + Width) {
      DirectPacker()(blockB + count, rhs.getSubMapper(j2 + Width, j2), end_k - (j2 + Width), Width);
      count += Width * (end_k - (j2 + Width));
    }
  }
  ...
  void operator()(Scalar* blockB, const Scalar* rhs_, Index rhsStride, Index rows, Index cols, Index k2) const {
    ...
    EIGEN_IF_CONSTEXPR (nr >= 4) {
      for (Index j2 = end8; j2 < numext::mini(end_k, packet_cols4); j2 += 4)
        pack_diagonal_panel<4>(blockB, rhs, rhs_t, k2, end_k, j2, count);
    }
