...
void testVectorType(const VectorType& base) {
  ...
  if (!NumTraits<Scalar>::IsInteger) {
    ...
    check_extremity_accuracy(m, low, high);
  }
  ...
  if ((!NumTraits<Scalar>::IsInteger) || (range_length >= size && (Index(range_length) % (size - 1)) == 0) ||
    ...
    if (!NumTraits<Scalar>::IsInteger) check_extremity_accuracy(m, low, high);
  }
...
TEST(NullaryTest, LinSpacedBoundary) {
  linspaced_boundary<float>();
  linspaced_boundary<double>();
}

// Integer LinSpaced divisor path tests.
TEST(NullaryTest, LinSpacedIntegerDivisor) { linspaced_integer_divisor<0>(); }
