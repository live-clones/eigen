...
void packetmath() {
  ...
  // Half and bfloat16 packets widen to float before reducing, so do not round the reference after every scalar
  // addition. REF_ADD preserves defined wraparound for signed integers and logical OR for bool, and avoids the MSVC
  // C4804 "unsafe use of bool" warning that raw operator+ triggers.
  using ReduxReferenceScalar =
      std::conditional_t<std::is_same<Scalar, half>::value || std::is_same<Scalar, bfloat16>::value, float, Scalar>;
