...
struct ptrue_impl {
  static EIGEN_DEVICE_FUNC inline Packet run(const Packet& /*a*/) {
    ...
    constexpr uint8_t kTrueByte = std::is_same<typename unpacket_traits<Packet>::type, bool>::value ? static_cast<uint8_t>(true) : 0xFF;
...
template <>
struct bit_and<bool> {
  EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR EIGEN_ALWAYS_INLINE bool operator()(const bool& a, const bool& b) const { return a && b; }
};
