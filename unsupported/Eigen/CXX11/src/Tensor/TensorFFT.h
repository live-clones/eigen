...
struct MakeComplex<true> {
  template <typename T>
  EIGEN_DEVICE_FUNC constexpr std::complex<T> operator()(const T& val) const {
...
struct MakeComplex<false> {
  template <typename T>
  EIGEN_DEVICE_FUNC constexpr std::complex<T> operator()(const std::complex<T>& val) const {
    return val;
  }
};
...
struct PartOf<RealPart> {
  template <typename T>
  constexpr T operator()(const std::complex<T>& val) const {
    return val.real();
  }
};
