...
class ProductImpl<Lhs, Rhs, Option, HermitianShape> : public internal::hermitian_product_base<Lhs, Rhs, Option> {
 private:
  using NestedExpression = typename Lhs::NestedExpression;
...
struct traits<HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension, 0>>
  ...
  typedef Matrix<_Scalar, _Dimension + 1, _Dimension / 2, _Storage, _MaxDimension + 1, _MaxDimension / 2>
      NestedExpression;
...
struct traits<HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension, 1>>
    : traits<Matrix<_Scalar, _Dimension, _Dimension, _Storage, _MaxDimension, _MaxDimension>> {
  typedef Matrix<_Scalar, _Dimension, (_Dimension + 1) / 2, _Storage, _MaxDimension, (_MaxDimension + 1) / 2>
      NestedExpression;
  typedef _Scalar Scalar;
  typedef Matrix<_Scalar, _Dimension, _Dimension, _Storage, _MaxDimension, _MaxDimension> DenseType;
  typedef HermitianMatrix<_Scalar, Dynamic, _UpLo, _Storage, _MaxDimension> PlainObject;
...
struct hermitian_prod_impl {
  ...
  static void run(DstXprType &dst, const Lhs &lhs, const Rhs &rhs) {
    ...
    const Matrix<Scalar, ((Lhs::Dimension == Eigen::Dynamic) ? (Eigen::Dynamic) : (Lhs::Dimension / 2)),
                 ((Lhs::Dimension == Eigen::Dynamic) ? (Eigen::Dynamic) : (Lhs::Dimension / 2))> &tmp =
        lhs.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template selfadjointView<Lower>();
    // const Matrix<Scalar, (Rhs::Dimension == Eigen::Dynamic) ? (Eigen::Dynamic) : (Rhs::Dimension / 2),
                 (Rhs::Dimension == Eigen::Dynamic) ? (Eigen::Dynamic) : (Rhs::Dimension / 2)> &tmp2 =
        lhs.nestedExpression().block(0, 0, dimension / 2, dimension / 2).template selfadjointView<Upper>();
#pragma omp parallel sections
    {
#pragma omp section
      {
        dst.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template triangularView<Lower>() =
            tmp * rhs.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template selfadjointView<Lower>() +
...
#pragma omp section
      {
        dst.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) =
            lhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) *
...
#pragma omp section
      {
        dst.nestedExpression().block(0, 0, dimension / 2, dimension / 2).template triangularView<Upper>() =
            lhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) *
...
class HermitianMatrix : public HermitianBase<HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>> {
 public:
#ifndef EIGEN_PARSED_BY_DOXYGEN
  ...
  enum { Dimension = _Dimension, UpLo = _UpLo };

  /** Default constructor without initialization */
  EIGEN_DEVICE_FUNC
  inline HermitianMatrix()
      : m_dimension(_Dimension == Dynamic ? 0 : _Dimension), m_odd(_Dimension == Dynamic ? 0 : _Dimension % 2) {}

  /**  Constructs a hermitian matrix with given dimension */
  EIGEN_DEVICE_FUNC
  explicit inline HermitianMatrix(Index dim) : m_odd(dim % 2), m_dimension(dim) {
    if (dim % 2) {
  ...
  explicit inline HermitianMatrix(const MatrixBase<OtherDerived> &other)
    ...
    EIGEN_STATIC_ASSERT(_UpLo == Upper || _UpLo == Lower, HERMITIANMATRIX_ACCEPTS_UPPER_AND_LOWER_MODE_ONLY);
  ...
  /** Copy constructor */
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC inline HermitianMatrix(const HermitianBase<OtherDerived> &other)
  ...
  explicit inline HermitianMatrix(const HermitianMatrix &other)
      : m_nested(other.nestedExpression()), m_dimension(other.m_dimension), m_odd(other.m_dimension % 2) {}
#endif

  template <typename OtherDerived>
  explicit inline HermitianMatrix(
      const CwiseBinaryOp<internal::scalar_sum_op<_Scalar, typename internal::traits<OtherDerived>::Scalar>,
                          const HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>,
                          const OtherDerived> &other)
      : m_dimension(other.lhs().cols()),
        m_odd(other.lhs().cols() % 2),
        m_nested(other.lhs().nestedExpression() + other.rhs().nestedExpression()) {}

  template <typename OtherDerived>
  explicit inline HermitianMatrix(
      const CwiseBinaryOp<internal::scalar_difference_op<_Scalar, typename internal::traits<OtherDerived>::Scalar>,
                          const HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>,
                          const OtherDerived> &other)
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC HermitianMatrix &operator=(const HermitianBase<OtherDerived> &other) {
    return Base::operator=(other);
  }

  /** Copy assignment constructor */
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC HermitianMatrix &operator=(const DenseBase<OtherDerived> &other) {
    return Base::operator=(other);
  }

  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC HermitianMatrix &operator=(
      const CwiseBinaryOp<internal::scalar_sum_op<_Scalar, typename internal::traits<OtherDerived>::Scalar>,
  ...
  template <typename OtherDerived>
  EIGEN_DEVICE_FUNC HermitianMatrix &operator=(
      const Product<const HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>, const OtherDerived,
                    LazyProduct> &product) {
    ...
    if (!m_odd) {
      if (_UpLo == Lower) {
        const typename HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>::NestedExpression
            &nestedLhs = product.lhs().nestedExpression();
        const typename OtherDerived::NestedExpression &nestedRhs = product.rhs().nestedExpression();

        const Matrix<_Scalar, Dynamic, Dynamic> &tmp =
            nestedLhs.block(1, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Lower>();
        const Matrix<_Scalar, Dynamic, Dynamic> &tmp2 =
            nestedLhs.block(0, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Upper>();
#pragma omp parallel sections
        {
#pragma omp section
          {
            this->nestedExpression().block(1, 0, m_dimension / 2, m_dimension / 2).template triangularView<Lower>() =
                tmp * nestedRhs.block(1, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Lower>() +
...
#pragma omp section
          {
            this->nestedExpression().block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) =
                nestedLhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) *
...
#pragma omp section
          {
            this->nestedExpression().block(0, 0, m_dimension / 2, m_dimension / 2).template triangularView<Upper>() =
                nestedLhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) *
                    nestedRhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2).transpose() +
                tmp2 * nestedRhs.block(0, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Upper>();
          }
        }
      } else if (_UpLo == Upper) {
        return *this;
  ...
  EIGEN_DEVICE_FUNC
  const NestedExpression &nestedExpression() const { return m_nested; }

  /** \returns a reference to the nested expression */
  EIGEN_DEVICE_FUNC
  NestedExpression &nestedExpression() { return m_nested; }
  ...
  EIGEN_DEVICE_FUNC
  inline void setZero() { m_nested.setZero(); }
...
template <>
struct storage_kind_to_shape<HermitianShape> {
  typedef HermitianShape Shape;
};
...
template <>
struct AssignmentKind<HermitianShape, HermitianShape> {
  typedef Hermitian2Hermitian Kind;
};
template <>
struct AssignmentKind<DenseShape, HermitianShape> {
  typedef Hermitian2Dense Kind;
};
template <>
struct AssignmentKind<HermitianShape, DenseShape> {
  typedef Dense2Hermitian Kind;
};
...
template <typename DstXprType, typename _Scalar, int _Dimension, unsigned int _UpLo, int _Storage, int _MaxDimension,
          typename Functor>
struct Assignment<DstXprType,
                  Product<HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>,
                          HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>, LazyProduct>,
                  Functor, Dense2Dense> {
  typedef Product<HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>,
                  HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>, LazyProduct>
      SrcXprType;
...
struct Assignment<DstXprType,
  ...
  static void run(DstXprType &dst, const SrcXprType &src,
                  const internal::assign_op<typename DstXprType::Scalar, typename SrcXprType::Scalar> & /*func*/) {
    hermitian_prod_impl<DstXprType, HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>,
                        HermitianMatrix<_Scalar, _Dimension, _UpLo, _Storage, _MaxDimension>>::run(dst, src.lhs(),
                                                                                                   src.rhs());
  }
};
...
EIGEN_DEVICE_FUNC inline const Product<Derived, HermitianDerived, LazyProduct> MatrixBase<Derived>::operator*(
    const HermitianBase<HermitianDerived> &hermitian) const {
