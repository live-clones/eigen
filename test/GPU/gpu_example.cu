...
EIGEN_DECLARE_TEST(gpu_example) {
  ...
  for (int i = 0; i < g_repeat; i++) {
    ...
    test_add(Eigen::Vector3f());
    test_add(Eigen::Matrix3d());
    test_add(Eigen::MatrixX<int>(10, 10));

    test_add(Eigen::Array44f());
    test_add(Eigen::ArrayXd(20));
    test_add(Eigen::ArrayXXi(13, 17));

    test_multiply(Eigen::Matrix3d(), Eigen::Matrix3d(), Eigen::Matrix3d());
    test_multiply(Eigen::MatrixX<int>(10, 10), Eigen::MatrixX<int>(10, 10), Eigen::MatrixX<int>());
    test_multiply(Eigen::MatrixXf(12, 1), Eigen::MatrixXf(1, 32), Eigen::MatrixXf());
  }
}
