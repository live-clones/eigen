...
void check_power_of_two_scaling_factor() {
  ...
  const auto check_factor = [](const T& value) {
    const auto factors = internal::safe_scaling<T>::compute_factors(value);
  ...
  const auto floorFactors = internal::safe_scaling<T>::compute_factors(T(1.5));
...
void check_arithmetic_safe_scaling_fallback() {
  ...
  const auto check_extreme_unscale = [](const double value) {
    const Factors extremeFactors = Scaling::compute_factors(value);
    double restored;
...
void check_subnormal_preserving_scaling() {
  ...
  const auto factors = internal::safe_scaling<RealScalar>::compute_factors(maxCoeff);
