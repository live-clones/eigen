...
class LLT : public SolverBase<LLT<MatrixType_, UpLo_> > {
 public:
   ...
   */
  EIGEN_DEVICE_FUNC LLT() : m_matrix(), m_l1_norm(0), m_isInitialized(false), m_info(InvalidInput) {}
   ...
   */
  template <typename InputType>
  EIGEN_DEVICE_FUNC explicit LLT(EigenBase<InputType>& matrix)
  ...
  EIGEN_DEVICE_FUNC inline Solve<LLT, Rhs> solve(const MatrixBase<Rhs>& b) const;
#endif

  template <typename Derived>
  EIGEN_DEVICE_FUNC void solveInPlace(const MatrixBase<Derived>& bAndX) const;

  template <typename InputType>
  EIGEN_DEVICE_FUNC LLT& compute(const EigenBase<InputType>& matrix);
   ...
   */
  EIGEN_DEVICE_FUNC inline const MatrixType& matrixLLT() const {
    eigen_assert(m_isInitialized && "LLT is not initialized.");
    return m_matrix;
  }
   ...
   */
  EIGEN_DEVICE_FUNC const LLT& adjoint() const noexcept { return *this; }
  ...
  EIGEN_DEVICE_FUNC LLT& rankUpdate(const VectorType& vec, const RealScalar& sigma = 1);
...
struct llt_inplace<Scalar, Lower> {
  ...
  EIGEN_DEVICE_FUNC static Index unblocked(MatrixType& mat) {
    using std::sqrt;
  ...
  template <typename MatrixType>
  EIGEN_DEVICE_FUNC static Index blocked(MatrixType& m) {
  ...
  template <typename MatrixType, typename VectorType>
  EIGEN_DEVICE_FUNC static Index rankUpdate(MatrixType& mat, const VectorType& vec, const RealScalar& sigma) {
...
struct llt_inplace<Scalar, Upper> {
  ...
  template <typename MatrixType>
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE Index unblocked(MatrixType& mat) {
    Transpose<MatrixType> matt(mat);
    return llt_inplace<Scalar, Lower>::unblocked(matt);
  }
  template <typename MatrixType>
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE Index blocked(MatrixType& mat) {
    Transpose<MatrixType> matt(mat);
...
template <typename InputType>
EIGEN_DEVICE_FUNC LLT<MatrixType, UpLo_>& LLT<MatrixType, UpLo_>::compute(const EigenBase<InputType>& a) {
...
template <typename MatrixType_, int UpLo_>
template <typename RhsType, typename DstType>
EIGEN_DEVICE_FUNC void LLT<MatrixType_, UpLo_>::_solve_impl(const RhsType& rhs, DstType& dst) const {
  _solve_impl_transposed<true>(rhs, dst);
}

template <typename MatrixType_, int UpLo_>
template <bool Conjugate, typename RhsType, typename DstType>
EIGEN_DEVICE_FUNC void LLT<MatrixType_, UpLo_>::_solve_impl_transposed(const RhsType& rhs, DstType& dst) const {
  dst = rhs;
...
template <typename MatrixType, int UpLo_>
template <typename Derived>
EIGEN_DEVICE_FUNC void LLT<MatrixType, UpLo_>::solveInPlace(const MatrixBase<Derived>& bAndX) const {
  eigen_assert(m_isInitialized && "LLT is not initialized.");
...
template <typename MatrixType, int UpLo_>
EIGEN_DEVICE_FUNC MatrixType LLT<MatrixType, UpLo_>::reconstructedMatrix() const {
  eigen_assert(m_isInitialized && "LLT is not initialized.");
 ...
 */
template <typename Derived>
EIGEN_DEVICE_FUNC inline LLT<typename MatrixBase<Derived>::PlainObject> MatrixBase<Derived>::llt() const {
