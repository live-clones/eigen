...
static void test_inflation_thread_pool() {
  ...
  // tasks; exercise it with a raw tensor and an expression argument.
  Tensor<float, 3, DataLayout> tensor(17, 5, 7);
  tensor.setRandom();
  ...
  Tensor<float, 3, DataLayout> inflated(33, 13, 7);
  inflated.device(device) = tensor.inflate(strides);

  Tensor<float, 3, DataLayout> inflated_expr(33, 13, 7);
  ...
  for (Index i = 0; i < inflated.dimension(0); ++i) {
    for (Index j = 0; j < inflated.dimension(1); ++j) {
      for (Index k = 0; k < inflated.dimension(2); ++k) {
        if (i % 2 == 0 && j % 3 == 0) {
          VERIFY_IS_EQUAL(inflated(i, j, k), tensor(i / 2, j / 3, k));
          VERIFY_IS_EQUAL(inflated_expr(i, j, k), tensor(i / 2, j / 3, k) + 1.0f);
        } else {
          VERIFY_IS_EQUAL(inflated(i, j, k), 0.0f);
          VERIFY_IS_EQUAL(inflated_expr(i, j, k), 0.0f);
        }
      }
    }
  }
