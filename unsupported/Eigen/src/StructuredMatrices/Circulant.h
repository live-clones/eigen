...
class Circulant : public EigenBase<Circulant<Scalar_, Size_>> {
 public:
  ...
  Matrix<Scalar, Size_, Rhs::ColsAtCompileTime> solve(const MatrixBase<Rhs>& b) const {
    ...
    if (!b.allFinite()) {
      ...
      // addProduct): apply the inverse operator -- itself a circulant matrix,
      // generated by the inverse DFT of the reciprocal symbol -- through the
      // direct kernel so Inf/NaN propagate entrywise.
      ...
      Circulant(icol, ComplexVector()).directProduct(x, b.derived(), Scalar(1));
      return x;
    }
 ...
 private:
   ...
   * by solve()'s direct fallback, whose generator is applied only through the
   * direct kernel. */
