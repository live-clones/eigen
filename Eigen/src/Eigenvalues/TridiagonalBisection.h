 ...
 *
 * Passed to SelfAdjointEigenSolver::computeFromTridiagonal() together with the
 * #UseBisection option. The Sturm-sequence bisection algorithm can compute an
 * arbitrary contiguous subset of the spectrum, which the implicit-QR algorithm
 * cannot. Three modes are provided:
...
struct EigenvalueRange {
  ...
  static EigenvalueRange all() { return EigenvalueRange{All, 0, 0, 0.0, 0.0}; }

  /** \returns a selector for the eigenvalues with 0-based indices in [il, iu). */
  static EigenvalueRange indices(Index il, Index iu) { return EigenvalueRange{ByIndex, il, iu, 0.0, 0.0}; }

  /** \returns a selector for the eigenvalues in the interval [vl, vu). */
  static EigenvalueRange values(double vl, double vu) { return EigenvalueRange{ByValue, 0, 0, vl, vu}; }
};
...
Index tridiagonal_bisection(const DiagType& diag, const SubdiagType& subdiag, const EigenvalueRange& range,
  ...
  const ArrayType alpha = diag.array() * inv_scale;
  const ArrayType beta_abs = (n >= 2) ? ArrayType(subdiag.array().abs() * inv_scale) : ArrayType(0);
  ...
  const RealScalar pivmin = numext::maxi(safemin, safemin * max_beta_sq);
  ...
  if (range.kind == EigenvalueRange::ByIndex) {
    ...
    const RealScalar vl = RealScalar(range.vl) * inv_scale;
    const RealScalar vu = RealScalar(range.vu) * inv_scale;
    // The eigenvalues in [vl, vu) are exactly those with indices in
    // [count(vl), count(vu)); a single 2-point Sturm batch gives both counts.
    RealScalar pts[2] = {vl, vu};
    RealScalar cnts[2];
    tridiagonal_sturm_counts<RealScalar>(alpha.data(), beta_sq.data(), n, pivmin, pts, cnts, 2);
    t_lo = Index(cnts[0]);
    t_hi = Index(cnts[1]);
