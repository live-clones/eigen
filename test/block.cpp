...
std::enable_if_t<is_col_major(MatrixType::Flags),void>
...
template <typename MatrixType>
std::enable_if_t<is_row_major(MatrixType::Flags),void>
check_left_top(const MatrixType& m,  Index r, Index c,
...
compare_using_data_and_stride(const MatrixType& m)
{
  ...
  for(int j=0;j<cols;++j)
    for(int i=0;i<rows;++i)
      VERIFY(m.coeff(i,j) == data[is_row_major(MatrixType::Flags)
