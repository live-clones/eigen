...
The GPU version reads like CPU Eigen with explicit upload/download. Each
expression maps to a single NVIDIA library call -- `operator*` dispatches to
cuBLAS GEMM, `.llt().solve()` dispatches to cuSOLVER potrf + potrs. If it
compiles, it is a single library call. Unsupported expressions are compile
errors.
...
- `fromHost()` -- Host data must outlive the copy
...
```cpp
GpuContext ctx;                             // creates dedicated stream + handles
GpuContext ctx(nullptr);                    // use CUDA default stream (stream 0)
GpuContext ctx(my_stream);                  // borrow existing stream (not owned)
GpuContext& def = GpuContext::threadLocal(); // per-thread default (lazy-created)
GpuContext::setThreadLocal(&ctx);           // override thread-local default
GpuContext::setThreadLocal(nullptr);        // restore lazy-created default
ctx.stream();                               // raw CUDA stream
ctx.cublasHandle();                         // raw cuBLAS handle
ctx.cusolverHandle();                       // raw cuSOLVER handle
```
...
Q^H) + `trsm` (back-substitute on R) -- Q is never formed explicitly. Same
`compute`/`solve`/`info` pattern as `GpuLLT`.
...
| Method | Description |
|--------|-------------|
| `GpuSVD(A, options)` | Construct and compute |
| `singularValues()` | Download singular values |
| `matrixU()` | Download U (requires `ComputeThinU` or `ComputeFullU`) |
| `matrixV()` | Download V (matches `JacobiSVD::matrixV()`) |
| `matrixVT()` | Download V^T (matches cuSOLVER directly) |
| `solve(B)` | Pseudoinverse solve |
...
| Method | Description |
|--------|-------------|
| `GpuSelfAdjointEigenSolver(A)` | Construct with eigenvectors |
| `GpuSelfAdjointEigenSolver(A, EigenvaluesOnly)` | Values only |
| `compute(A, mode)` | Compute from host or DeviceMatrix |
| `eigenvalues()` | Download eigenvalues (ascending order) |
| `eigenvectors()` | Download eigenvectors (columns) |
| `info()` | Lazy sync |
...
(debug asserts catch violations). The `.noalias()` method exists as a no-op
