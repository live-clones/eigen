...
// ---- 64-bit dimension dispatch ----------------------------------------------
// cuBLAS 12.0 added `_64` overloads taking int64_t dimensions for the whole
// wrapped surface. The cublasX* wrappers below take int64_t and route through
// EIGEN_CUBLAS_FN / to_blas_dim: on cuBLAS >= 12 dimensions pass through to
// the `_64` entry points unchanged; on older cuBLAS they narrow through
// to_blas_int, which asserts on overflow — a release build against pre-12
// cuBLAS silently truncates dimensions beyond INT_MAX, as before.
