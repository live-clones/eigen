...
// these same mappers. For Tensor sub-mappers (the hot path -- tensor
// contractions pack through this on both sides), loadPacket is a contiguous
// NEON load, so both loads and stores are vectorised. For inner-strided
// ColMajor blas mappers, NEON has no hardware gather: Eigen's pgather performs
// scalar source loads, while pstoreu still emits a contiguous vector store. The
// packet path assumes the mapper's packets advance the first index; that holds for tensor
// sub-mappers and ColMajor blas mappers, but not for inner-strided RowMajor
// blas mappers, whose packets run along the storage-inner second index -- the
// RowMajor dispatch passes vectorise = false for those (unreachable today:
// the only Incr != 1 producer, TriangularSolverMatrix, is ColMajor-only) and
// they take the scalar element loop. Streaming SME is deliberately not used
// here: streaming-mode gathers need FEAT_SME_FA64 (absent on e.g. Apple M4),
// so non-contiguous sources cannot be loaded in streaming mode at all.
...
// lvalue mappers. VectoriseStrided is the inner-strided blas-mapper flag
// (by-value tensor sub-mappers always vectorise).
template <bool VectoriseStrided, typename Index, typename DataMapper, typename DirectFn, typename FallbackFn>
EIGEN_ALWAYS_INLINE void sme_dispatch_pack(DirectFn direct, FallbackFn fallback, float* block, const DataMapper& m,
  ...
  if (sme_mapper_incr<Index>(m) == 1) {
    ...
    fallback(block, m, depth, n, stride, offset, VectoriseStrided);
  }
}
template <bool VectoriseStrided, typename Index, typename DataMapper, typename DirectFn, typename FallbackFn>
EIGEN_ALWAYS_INLINE void sme_dispatch_pack(DirectFn, FallbackFn fallback, float* block, const DataMapper& m,
  ...
  fallback(block, m, depth, n, stride, offset, /*vectorise=*/true);
}
...
// Only float x float runs the SME gebp_kernel (and wants its blocking
// budgets); see the primary template next to
// evaluateProductBlockingSizesHeuristicForSme. Specialized here so the set of
// SME-accelerated scalar pairs lives with the kernel that defines it.
template <>
struct is_sme_gebp_kernel_pair<float, float> : std::true_type {};
...
struct gemm_pack_lhs<float, Index, DataMapper, kSmeMr, Pack2, Packet, ColMajor, Conjugate, PanelMode> {
  ...
  static_assert(!Conjugate || !NumTraits<Scalar>::IsComplex,
                "the SME packers ignore Conjugate; that is only sound for real scalars");
  ...
  EIGEN_DONT_INLINE void operator()(Scalar* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride = 0,
    ...
    // VectoriseStrided: inner-strided ColMajor blas mappers' packets advance
    // the row index, so the fallback may use them.
...
struct gemm_pack_lhs<float, Index, DataMapper, kSmeMr, Pack2, Packet, RowMajor, Conjugate, PanelMode> {
  ...
  EIGEN_DONT_INLINE void operator()(Scalar* blockA, const DataMapper& lhs, Index depth, Index rows, Index stride = 0,
    ...
    // !VectoriseStrided: inner-strided RowMajor blas mappers' packets advance
    // the depth index, not the row index -- the fallback must stay scalar
    // (see sme_pack_lhs_fallback).
...
struct gemm_pack_rhs<float, Index, DataMapper, kSmeNr, ColMajor, Conjugate, PanelMode> {
  ...
  EIGEN_DONT_INLINE void operator()(Scalar* blockB, const DataMapper& rhs, Index depth, Index cols, Index stride = 0,
    ...
    // VectoriseStrided: inner-strided ColMajor blas mappers' LinearMapper
    // packets advance the depth index, which is what the fallback transposes.
...
struct gemm_pack_rhs<float, Index, DataMapper, kSmeNr, RowMajor, Conjugate, PanelMode> {
  ...
  EIGEN_DONT_INLINE void operator()(Scalar* blockB, const DataMapper& rhs, Index depth, Index cols, Index stride = 0,
    ...
    // !VectoriseStrided: inner-strided RowMajor blas mappers' LinearMapper
    // packets advance the column index, not depth -- the fallback must stay
    // scalar (see sme_pack_rhs_fallback).
