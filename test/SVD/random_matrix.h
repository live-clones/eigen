...
#ifndef EIGEN_TEST_SVD_RANDOM_MATRIX_H
#define EIGEN_TEST_SVD_RANDOM_MATRIX_H
...
void check_setupRandomSvs(const Index dim, const RealScalarType max) {
  ...
  // check entries
  for (Index i = 0; i < v.size(); ++i) EXPECT_GE(v(i), 0);
  for (Index i = 0; i < v.size() - 1; ++i) EXPECT_GE(v(i), v(i + 1));
}
...
#endif  // EIGEN_TEST_SVD_RANDOM_MATRIX_H
