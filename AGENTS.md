...
1. **Provenance and attribution — no plagiarism, no license laundering.** Eigen code must be original or derived from publicly published material under an MPL-2.0-compatible license. Do **not** copy code — verbatim, paraphrased, or "translated" to another syntax — from incompatibly-licensed sources (proprietary, NDA-encumbered, prior-employer internal, or anything whose terms conflict with MPL-2.0 redistribution); paraphrasing is still a derivative work. **Cite published references inline** when they inform an implementation — LAPACK routines, LAPACK Working Notes (LAWN), ACM TOMS / SIAM papers, Higham, Golub & van Loan, textbook algorithms, Boost components, vendor application notes — by name (author, year, identifier; a comment or Doxygen `\note` block suffices). Same discipline applies to AI-suggested code: identify and cite the source, or rewrite from a known reference and cite that, or drop the suggestion. Ideas aren't copyrightable, but specific expressions are — learn from the source, write your own, credit it.
