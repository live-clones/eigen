...
#include "../plugins/BlockMethods.inc"
  // Indexed view methods (inlined from IndexedViewMethods.inc)
 ...
 public:
  ...
  using EnableOverload = std::enable_if_t<internal::valid_indexed_view_overload<RowIndices, ColIndices>::value &&
 ...
 public:
  ...
  template <typename RowIndices, typename ColType, size_t ColSize, typename ColIndices = Array<ColType, ColSize, 1>,
            EnableOverload<RowIndices, ColIndices> = true>
  IndexedViewType<RowIndices, ColIndices> operator()(const RowIndices& rowIndices,
   ...
   *
   * The last statement implies compatibility with \c std::vector, \c std::valarray, \c std::array, many of the
   * Range-v3's ranges, etc.
   *
   * If the submatrix can be represented using a starting position \c (i,j) and positive sizes \c (rows,columns), then
   * this method will returns a Block object after extraction of the relevant information from the passed arguments.
   * This is the case when all arguments are either:
   *  - An integer
