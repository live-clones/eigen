...
EIGEN_DECLARE_TEST(product_sme) {
  // Only test float, as that's what our SME kernel implementation supports currently
  ...
  for (int i = 0; i < g_repeat; i++) {
    CALL_SUBTEST_1( product(Matrix<float, Dynamic, Dynamic>(internal::random<int>(1, EIGEN_TEST_MAX_SIZE), internal::random<int>(1, EIGEN_TEST_MAX_SIZE))) );
  }
}
