...
template <typename T>
constexpr T incomplete_beta_inv_initial_val_1(const T alpha_par, const T beta_par, const T t_val,
                                              const T sgn_term) noexcept {  // a > 1.0
  return incomplete_beta_inv_initial_val_1_int_end(
      alpha_par, beta_par,
...
template <typename T>
constexpr T incomplete_beta_inv_initial_val_2(const T alpha_par, const T beta_par, const T p, const T s_1,
...
constexpr T incomplete_beta_inv_initial_val(const T alpha_par, const T beta_par, const T p) noexcept {
  return ((alpha_par > T(1) && beta_par > T(1)) ?
                                                // if
              incomplete_beta_inv_initial_val_1(alpha_par, beta_par, incomplete_beta_inv_initial_val_1_tval(p),
                                                p < T(0.5) ? T(1) : T(-1))
                                                :
                                                // else
              p > T(0.5) ?
...
template <typename T>
constexpr T incomplete_beta_inv_err_val(const T value, const T alpha_par, const T beta_par,
                                        const T p) noexcept {  // err_val = f(x)
  return (incomplete_beta(alpha_par, beta_par, value) - p);
}

template <typename T>
constexpr T incomplete_beta_inv_deriv_1(
    const T value, const T alpha_par, const T beta_par,
    const T lb_val) noexcept {  // derivative of the incomplete beta function w.r.t. x
  return (                      // indistinguishable from zero or one
      GCLIM<T>::min() > abs(value)          ? T(0)
      : GCLIM<T>::min() > abs(T(1) - value) ? T(0)
                                            :
                                            // else
...
template <typename T>
constexpr T incomplete_beta_inv_deriv_2(
    const T value, const T alpha_par, const T beta_par,
...
template <typename T>
constexpr T incomplete_beta_inv_ratio_val_1(const T value, const T alpha_par, const T beta_par, const T p,
                                            const T deriv_1) noexcept {
  return (incomplete_beta_inv_err_val(value, alpha_par, beta_par, p) / deriv_1);
}

template <typename T>
constexpr T incomplete_beta_inv_ratio_val_2(const T value, const T alpha_par, const T beta_par,
                                            const T deriv_1) noexcept {
...
constexpr T incomplete_beta_inv_recur(const T value, const T alpha_par, const T beta_par, const T p, const T deriv_1,
  ...
  return (  // derivative = 0
      GCLIM<T>::min() > abs(deriv_1)
          ? incomplete_beta_inv_decision(value, alpha_par, beta_par, p, T(0), lb_val, GCEM_INCML_BETA_INV_MAX_ITER + 1)
          :
          // else
          incomplete_beta_inv_decision(
              value, alpha_par, beta_par, p,
...
constexpr T incomplete_beta_inv_decision(const T value, const T alpha_par, const T beta_par, const T p, const T direc,
  ...
  return (iter_count <= GCEM_INCML_BETA_INV_MAX_ITER ?
              ...
              incomplete_beta_inv_recur(value - direc, alpha_par, beta_par, p,
                                        incomplete_beta_inv_deriv_1(value, alpha_par, beta_par, lb_val), lb_val,
                                        iter_count + 1)
                                                     :
                                                     // else
              value - direc);
...
template <typename T>
constexpr T incomplete_beta_inv_begin(const T initial_val, const T alpha_par, const T beta_par, const T p,
