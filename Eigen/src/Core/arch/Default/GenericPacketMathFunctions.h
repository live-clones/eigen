...
EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS
  ...
  if (EIGEN_PREDICT_FALSE(predux_any(pcmp_le(pset1<Packet>(small_th), x_abs)))) {
    ...
    Packet q_low = pround(psub(pmul(x_abs, cst_2oPI), q_high));
    q_int = pcast<Packet, PacketI>(q_low);
    s = trig_reduce_medium_double(x_abs, q_high, q_low);
  } else {
    Packet qval = pround(pmul(x_abs, cst_2oPI));
    s = trig_reduce_small_double(x_abs, qval);
    q_int = pcast<Packet, PacketI>(qval);
  }
  ...
  // TODO Remove it when huge angle argument reduction will be implemented
  if (EIGEN_PREDICT_FALSE(predux_any(pcmp_le(pset1<Packet>(huge_th), x_abs)))) {
    ...
    for (int k = 0; k < PacketSize; ++k) {
      double val = x_cpy[k];
      if (x_abs[k] > huge_th && (numext::isfinite)(val)) {
        if (ComputeBoth)
          sincos_vals[k] = k % 2 == 0 ? std::sin(x_cpy[k]) : std::cos(x_cpy[k]);
        else
          sincos_vals[k] = ComputeSine ? std::sin(x_cpy[k]) : std::cos(x_cpy[k]);
      }
    }
