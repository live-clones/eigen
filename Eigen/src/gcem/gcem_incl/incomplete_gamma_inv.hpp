...
template <typename T>
constexpr T incomplete_gamma_inv_decision(const T value, const T a, const T p, const T direc, const T lg_val,
...
constexpr T incomplete_gamma_inv_deriv_2(
    ...
    const T deriv_1) noexcept {  // second derivative of the incomplete gamma function w.r.t. x
...
template <typename T>
constexpr T incomplete_gamma_inv_recur(const T value, const T a, const T p, const T deriv_1, const T lg_val,
                                       const int iter_count) noexcept {
...
constexpr T incomplete_gamma_inv_decision(const T value, const T a, const T p, const T direc, const T lg_val,
  ...
  return (iter_count <= GCEM_INCML_GAMMA_INV_MAX_ITER ?  // if
              incomplete_gamma_inv_recur(value - direc, a, p, incomplete_gamma_inv_deriv_1(value, a, lg_val), lg_val,
                                         iter_count + 1)
                                                      :
                                                      // else
              value - direc);
...
constexpr T incomplete_gamma_inv_check(const T a, const T p) noexcept {
  return (  // NaN check
      ...
      : GCLIM<T>::min() > abs(T(1) - p) ? GCLIM<T>::infinity()
                                        :
                                        //
          GCLIM<T>::min() > a ? T(0)
                              :
