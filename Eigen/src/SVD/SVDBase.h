...
class SVDBase : public SolverBase<SVDBase<Derived> > {
 public:
  ...
  constexpr bool computeU() const { return m_computeFullU || m_computeThinU; }
  /** \returns true if \a V (full or thin) is asked for in this SVD decomposition */
  constexpr bool computeV() const { return m_computeFullV || m_computeThinV; }

  constexpr Index rows() const { return m_rows.value(); }
  constexpr Index cols() const { return m_cols.value(); }
  constexpr Index diagSize() const { return m_diagSize.value(); }
