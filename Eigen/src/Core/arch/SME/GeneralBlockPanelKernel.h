  ...
  for (Index j = 0; j < cols; j += NR) {
    ...
    for (; c < peeled_w; c += Index(PacketSize)) {
      ...
      for (; k < peeled_depth; k += Index(PacketSize)) {
        // The sub-mappers are loop-invariant in k and cost a pointer offset,
        // so they are formed here rather than threaded through the k loop.
        PacketBlock<Packet, PacketSize> block;
        for (int p = 0; p < PacketSize; ++p) {
          block.packet[p] = rhs.getLinearMapper(0, j + c + Index(p)).template loadPacket<Packet>(k);
        }
        ptranspose(block);
        for (int p = 0; p < PacketSize; ++p) {
