...
EIGEN_DECLARE_TEST(rand) {
  ...
  for (int i = 0; i < g_repeat * 10000; i++) {
    ...
    check_in_range<float>(-1.0f, 1.0f);
    check_in_range<float>(-1432.2352f, -1432.2352f);

    check_in_range<double>(10.0, 11.0);
    check_in_range<double>(1.24234523, 1.24234523);
    check_in_range<double>(-1.0, 1.0);
    check_in_range<double>(-1432.2352, -1432.2352);

    check_in_range<long double>(10.0L, 11.0L);
    check_in_range<long double>(1.24234523L, 1.24234523L);
    check_in_range<long double>(-1.0L, 1.0L);
    check_in_range<long double>(-1432.2352L, -1432.2352L);

    check_in_range<half>(half(10.0f), half(11.0f));
    check_in_range<half>(half(1.24234523f), half(1.24234523f));
    check_in_range<half>(half(-1.0f), half(1.0f));
    check_in_range<half>(half(-1432.2352f), half(-1432.2352f));

    check_in_range<bfloat16>(bfloat16(10.0f), bfloat16(11.0f));
    ...
    check_in_range<int32_t>(-673456, 673456);
    check_in_range<int32_t>(-RAND_MAX + 10, RAND_MAX - 10);
  ...
  check_all_in_range<int8_t>(-11 - int8t_offset, -11);
  check_all_in_range<int8_t>(-126, -126 + int8t_offset);
  ...
  check_all_in_range<int16_t>(-11 - int16t_offset, -11);
  ...
  check_all_in_range<int32_t>(-11 - g_repeat, -11);
  check_all_in_range<int32_t>(-673456, -673456 + g_repeat);
  check_all_in_range<int32_t>(673456, 673456 + g_repeat);

  check_all_in_range<int64_t>(11, 11);
  check_all_in_range<int64_t>(11, 11 + g_repeat);
  check_all_in_range<int64_t>(-5, 5);
  ...
  check_histogram<int32_t>(-3333, -3333 + bins * (3333 / bins) - 1, bins);
  bins = 1000;
  check_histogram<int32_t>(-RAND_MAX + 10, -RAND_MAX + 10 + bins * (RAND_MAX / bins) - 1, bins);
  check_histogram<int32_t>(-RAND_MAX + 10, -int64_t(RAND_MAX) + 10 + bins * (2 * int64_t(RAND_MAX) / bins) - 1, bins);

  check_histogram<bool>(/*bins=*/2);
  check_histogram<uint8_t>(/*bins=*/16);
  check_histogram<uint16_t>(/*bins=*/1024);
  check_histogram<uint32_t>(/*bins=*/1024);
  check_histogram<uint64_t>(/*bins=*/1024);

  check_histogram<int8_t>(/*bins=*/16);
  ...
  check_histogram<half>(half(-10.0f), half(10.0f), /*bins=*/512);
  check_histogram<bfloat16>(bfloat16(-10.0f), bfloat16(10.0f), /*bins=*/64);
  check_histogram<SafeScalar<float>>(-10.0f, 10.0f, /*bins=*/1024);
  check_histogram<SafeScalar<half>>(half(-10.0f), half(10.0f), /*bins=*/512);
  check_histogram<SafeScalar<bfloat16>>(bfloat16(-10.0f), bfloat16(10.0f), /*bins=*/64);

  check_histogram<float>(/*bins=*/1024);
  check_histogram<double>(/*bins=*/1024);
  check_histogram<long double>(/*bins=*/1024);
  check_histogram<half>(/*bins=*/512);
  check_histogram<bfloat16>(/*bins=*/64);
  check_histogram<SafeScalar<float>>(/*bins=*/1024);
  check_histogram<SafeScalar<half>>(/*bins=*/512);
  check_histogram<SafeScalar<bfloat16>>(/*bins=*/64);
}
