...
EIGEN_STRONG_INLINE Packet8f pcast<Packet16h, Packet8f>(const Packet16h& a) {
  // Discard second-half of input.
  Packet8h low = _mm_castps_ph(_mm256_extractf32x4_ps(_mm256_castsi256_ps(a), 0));
...
EIGEN_STRONG_INLINE Packet16h pcast<Packet8f, Packet16h>(const Packet8f& a, const Packet8f& b) {
  ...
  result = _mm256_insertf64x2(
      result, _mm_castsi128_pd(_mm256_cvtps_ph(_mm256_castph_si256(a), _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)),
      0);
  result = _mm256_insertf64x2(
      result, _mm_castsi128_pd(_mm256_cvtps_ph(_mm256_castph_si256(b), _MM_FROUND_TO_NEAREST_INT | _MM_FROUND_NO_EXC)),
      1);
  return _mm256_castpd_ph(result);
}
template <>
EIGEN_STRONG_INLINE Packet8h pcast<Packet4f, Packet8h>(const Packet4f& a, const Packet4f& b) {
  ...
  return _mm_castsi128_ph(_mm256_cvtps_ph(result, _MM_FROUND_TO_NEAREST_INT));
}
