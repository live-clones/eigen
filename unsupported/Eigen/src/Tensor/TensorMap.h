...
class TensorMap : public TensorBase<TensorMap<PlainObjectType, Options_, MakePointer_> > {
 public:
  ...
  EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE StorageRefType
  operator()(const array<Index, NumIndices>& indices) const {
  ...
  EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE StorageRefType operator()(const array<Index, NumIndices>& indices) {
    if (PlainObjectType::Options & RowMajor) {
      const Index index = m_dimensions.IndexOfRowMajor(indices);
      return m_data[index];
    } else {
