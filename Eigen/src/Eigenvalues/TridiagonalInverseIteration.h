...
/** \internal
 *
 * Self-contained 64-bit linear congruential generator (the MMIX multiplier/increment)
 * producing reproducible pseudo-random values in the open interval (-1, 1). Used to seed
 * the inverse-iteration start vectors.
 *
...
void tridiagonal_inverse_iteration(const DiagType& diag, const SubdiagType& subdiag, const EivalType& eivals,
  ...
  for (Index j = 0; j < m; ++j) {
    ...
    for (int its = 0; its < maxits; ++its) {
      ...
      Index jmax = 0;
      b.cwiseAbs().maxCoeff(&jmax);
      if (numext::is_exactly_zero(b[jmax])) break;  // degenerate iterate; accept as is
      const RealScalar scl = RealScalar(n) * onenrm * numext::maxi(eps, abs(lu_d[n - 1])) / abs(b[jmax]);
      b *= scl;
      ...
      b.cwiseAbs().maxCoeff(&jmax);
      const RealScalar nrm = abs(b[jmax]);
    ...
    Index jmax = 0;
    b.cwiseAbs().maxCoeff(&jmax);
    const RealScalar binf = abs(b[jmax]);
