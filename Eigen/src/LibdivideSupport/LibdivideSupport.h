...
#ifdef LIBDIVIDE_H
EIGEN_STATIC_ASSERT(std::false_type::value, INCLUDE_THIS_FILE_BEFORE_LIBDIVIDE.H)
#endif  // LIBDIVIDE_H
...
#include "libdivide.h"//#include <libdivide.h>//change back to #include <libdivide.h> after test
...
namespace internal {
    ...
    struct integer_is_representable {
        static constexpr bool value = ( Eigen::NumTraits<SrcScalar>::IsInteger && Eigen::NumTraits<DstScalar>::IsInteger) &&
...
template <typename SrcScalar, typename DstScalar>
static constexpr bool integer_is_representable_v = integer_is_representable<SrcScalar, DstScalar>::value;
...
using CwiseLibdivideReturnType = std::enable_if_t<
    internal::is_valid_libdivide_op<typename Derived::Scalar, DivisorScalar>,
    CwiseUnaryOp<internal::scalar_libdivide_op<typename Derived::Scalar, DivisorScalar>, const Derived>>;

template <typename Derived, typename DivisorScalar>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CwiseLibdivideReturnType<Derived, DivisorScalar> libdivideQuotient(
    const DenseBase<Derived>& lhs, const DivisorScalar& rhs) {
...
template <typename Derived, typename DivisorScalar>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CwiseLibdivideReturnType<Derived, DivisorScalar> operator/(
