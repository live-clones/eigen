...
class ThreadPoolTempl : public Eigen::ThreadPoolInterface {
 public:
  ...
  ThreadPoolTempl(int num_threads, int max_spinning_threads,
                  int min_spinning_duration_microseconds = 0,
                  Environment env = Environment())
      : env_(env),
        num_threads_(num_threads),
        max_spinning_threads_(max_spinning_threads),
        min_spinning_duration_microseconds_(min_spinning_duration_microseconds),
    ...
    eigen_assert(min_spinning_duration_microseconds_ >= 0 && "The minimum spinning duration must be >= 0");
 ...
 private:
  ...
  const int min_spinning_duration_microseconds_;
  ...
  EIGEN_ALWAYS_INLINE void StopSpinning() {
    if (max_spinning_threads_ < num_threads_) {
      ...
      while (num_spinning > 0 && 
             !spinning_.compare_exchange_weak(num_spinning, num_spinning - 1,
                                              std::memory_order_relaxed)) {
      }
    }
  }
  ...
  EIGEN_ALWAYS_INLINE Task TryGrabbingTask(int spin_count, GrabTaskFunctor grab_task) {
    ...
    if (!t.f && max_spinning_threads_ > 0) {
      // To reduce sys-call overhead, only call clock routines when
      // necessary.       
      std::chrono::time_point<std::chrono::steady_clock> spin_start;
      int spin_duration = 0;
      if (min_spinning_duration_microseconds_ > 0) {
        spin_start = std::chrono::steady_clock::now();
      }
      do {
        // Iteratively try a fixed number of times for speed.
        ...
        if (min_spinning_duration_microseconds_ > 0) {
          ...
          spin_duration = static_cast<int>(
              std::chrono::duration_cast<std::chrono::microseconds>(spin_stop - spin_start).count());
        }
      } while (spin_duration < min_spinning_duration_microseconds_);
    }
    return t;
  }
