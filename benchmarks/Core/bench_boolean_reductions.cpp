...
enum InputPattern { Random, AllFalse, AllTrue, FirstTrue, LastTrue, FirstFalse, LastFalse };

template <InputPattern Pattern>
void fill_bool_input(BoolArray& input) {
  std::uint32_t state = 0x12345678u;
  for (Index i = 0; i < input.size(); ++i) {
    state = state * 1664525u + 1013904223u;
    input[i] = Pattern == AllTrue || Pattern == FirstFalse || Pattern == LastFalse ||
               (Pattern == Random && (state >> 31) != 0);
  }
...
void BM_PacketReduction(benchmark::State& state) {
  ...
  EIGEN_ALIGN_TO_BOUNDARY(internal::unpacket_traits<Packet>::alignment) Scalar input[PacketSize];
