...
TEST(MappedMatrixTest, InnerStrideBoundary) {
  // Strided map tests at vectorization boundaries (deterministic, outside g_repeat).
  map_inner_stride_boundary<float>();
  map_inner_stride_boundary<double>();
}

TEST(MappedMatrixTest, OuterStrideBoundary) {
  map_outer_stride_boundary<float>();
  map_outer_stride_boundary<double>();
}
