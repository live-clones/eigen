...
class CompressedStorage {
 ...
 public:
  ...
  constexpr Index allocatedSize() const { return m_allocatedSize; }
  ...
  constexpr Scalar& value(Index i) {
    eigen_internal_assert(m_values != 0);
    return m_values[i];
  }
  constexpr const Scalar& value(Index i) const {
    eigen_internal_assert(m_values != 0);
    return m_values[i];
  }

  constexpr StorageIndex& index(Index i) {
    eigen_internal_assert(m_indices != 0);
    return m_indices[i];
  }
  constexpr const StorageIndex& index(Index i) const {
    eigen_internal_assert(m_indices != 0);
    return m_indices[i];
  }

  /** \returns the largest \c k such that for all \c j in [0,k) index[\c j]\<\a key */
  constexpr Index searchLowerIndex(Index key) const { return searchLowerIndex(0, m_size, key); }

  /** \returns the largest \c k in [start,end) such that for all \c j in [start,k) index[\c j]\<\a key */
  constexpr Index searchLowerIndex(Index start, Index end, Index key) const {
  ...
  constexpr Scalar at(Index key, const Scalar& defaultValue = Scalar(0)) const {
    if (m_size == 0)
      return defaultValue;
  ...
  constexpr Scalar atInRange(Index start, Index end, Index key, const Scalar& defaultValue = Scalar(0)) const {
    if (start >= end)
      return defaultValue;
  ...
  constexpr Scalar& atWithInsertion(Index key, const Scalar& defaultValue = Scalar(0)) {
  ...
  constexpr void moveChunk(Index from, Index to, Index chunkSize) {
