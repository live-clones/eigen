...
EIGEN_DECLARE_TEST(gpu_cholesky) {
  ...
  out_h.setConstant(data_size, static_cast<Eigen::half>(-1));
#endif
  ...
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_1((run_and_compare_to_gpu(llt_reconstruct<Matrix<double, 5, 5>>(), nthreads, in_d, out_d)));
    CALL_SUBTEST_1((run_and_compare_to_gpu(llt_reconstruct<Matrix<Eigen::half, 5, 5>>(), nthreads, in_h, out_h)));
    ...
    CALL_SUBTEST_1((run_and_compare_to_gpu(ldlt_reconstruct<Matrix<double, 5, 5>>(), nthreads, in_d, out_d)));
    CALL_SUBTEST_1((run_and_compare_to_gpu(ldlt_reconstruct<Matrix<Eigen::half, 5, 5>>(), nthreads, in_h, out_h)));
    ...
    CALL_SUBTEST_1((run_and_compare_to_gpu(bunch_kaufman_reconstruct<Matrix<double, 5, 5>>(), nthreads, in_d, out_d)));
    CALL_SUBTEST_1((run_and_compare_to_gpu(bunch_kaufman_reconstruct<Matrix<Eigen::half, 5, 5>>(), nthreads, in_h, out_h)));
    ...
    CALL_SUBTEST_1((run_and_compare_to_gpu(llt_solve<Matrix<double, 5, 5>>(), nthreads, in_d, out_d)));
    CALL_SUBTEST_1((run_and_compare_to_gpu(llt_solve<Matrix<Eigen::half, 5, 5>>(), nthreads, in_h, out_h)));
  }
}
