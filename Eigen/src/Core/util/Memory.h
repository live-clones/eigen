...
EIGEN_DEVICE_FUNC constexpr void check_that_malloc_is_allowed() {
  eigen_assert(false && "heap allocation is forbidden (EIGEN_NO_MALLOC is defined)");
}
EIGEN_DEVICE_FUNC constexpr void check_that_free_is_allowed() {
  eigen_assert(false && "heap deallocation is forbidden (EIGEN_NO_MALLOC is defined)");
}
#elif defined EIGEN_RUNTIME_NO_MALLOC
EIGEN_DEVICE_FUNC constexpr bool is_malloc_allowed_impl(bool update, bool new_value = false) {
  EIGEN_MALLOC_CHECK_THREAD_LOCAL static bool value = true;
  if (update == 1) value = new_value;
  return value;
}
EIGEN_DEVICE_FUNC constexpr bool is_malloc_allowed() { return is_malloc_allowed_impl(false); }
EIGEN_DEVICE_FUNC constexpr bool set_is_malloc_allowed(bool new_value) {
...
EIGEN_DEVICE_FUNC constexpr bool is_free_allowed_impl(bool update, bool new_value = false) {
  EIGEN_MALLOC_CHECK_THREAD_LOCAL static bool value = true;
  if (update == 1) value = new_value;
  return value;
}
EIGEN_DEVICE_FUNC constexpr bool is_free_allowed() { return is_free_allowed_impl(false); }
EIGEN_DEVICE_FUNC constexpr bool set_is_free_allowed(bool new_value) { return is_free_allowed_impl(true, new_value); }
EIGEN_DEVICE_FUNC constexpr void check_that_free_is_allowed() {
...
EIGEN_DEVICE_FUNC constexpr void throw_std_bad_alloc() {
 ...
 */
EIGEN_DEVICE_FUNC constexpr void* handmade_aligned_malloc(std::size_t size,
...
EIGEN_DEVICE_FUNC constexpr void handmade_aligned_free(void* ptr) {
  if (ptr != nullptr) {
 ...
 */
EIGEN_DEVICE_FUNC constexpr void* handmade_aligned_realloc(void* ptr, std::size_t new_size, std::size_t old_size,
                                                           std::size_t alignment = EIGEN_DEFAULT_ALIGN_BYTES) {
 ...
 */
EIGEN_DEVICE_FUNC constexpr void* aligned_malloc(std::size_t size) {
...
EIGEN_DEVICE_FUNC constexpr void aligned_free(void* ptr) {
#if (EIGEN_DEFAULT_ALIGN_BYTES == 0) || EIGEN_MALLOC_ALREADY_ALIGNED
 ...
 */
EIGEN_DEVICE_FUNC constexpr void* aligned_realloc(void* ptr, std::size_t new_size, std::size_t old_size) {
 ...
 */
template <bool Align>
EIGEN_DEVICE_FUNC constexpr void* conditional_aligned_malloc(std::size_t size) {
  return aligned_malloc(size);
}

template <>
EIGEN_DEVICE_FUNC constexpr void* conditional_aligned_malloc<false>(std::size_t size) {
  if (size == 0) return nullptr;
...
template <bool Align>
EIGEN_DEVICE_FUNC constexpr void conditional_aligned_free(void* ptr) {
  aligned_free(ptr);
}

template <>
EIGEN_DEVICE_FUNC constexpr void conditional_aligned_free<false>(void* ptr) {
  if (ptr != nullptr) {
    check_that_free_is_allowed();
    EIGEN_USING_STD(free)
    free(ptr);
...
template <bool Align>
EIGEN_DEVICE_FUNC constexpr void* conditional_aligned_realloc(void* ptr, std::size_t new_size, std::size_t old_size) {
  return aligned_realloc(ptr, new_size, old_size);
}

template <>
EIGEN_DEVICE_FUNC constexpr void* conditional_aligned_realloc<false>(void* ptr, std::size_t new_size,
                                                                     std::size_t old_size) {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr void destruct_elements_of_array(T* ptr, std::size_t size) {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr T* default_construct_elements_of_array(T* ptr, std::size_t size) {
  std::size_t i = 0;
  EIGEN_TRY {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr T* copy_construct_elements_of_array(T* ptr, const T* src, std::size_t size) {
  std::size_t i = 0;
  EIGEN_TRY {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr T* move_construct_elements_of_array(T* ptr, T* src, std::size_t size) {
  std::size_t i = 0;
  EIGEN_TRY {
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr T* aligned_new(std::size_t size) {
...
EIGEN_DEVICE_FUNC constexpr T* conditional_aligned_new(std::size_t size) {
  check_size_for_overflow<T>(size);
 ...
 */
template <typename T>
EIGEN_DEVICE_FUNC constexpr void aligned_delete(T* ptr, std::size_t size) {
 ...
 */
template <typename T, bool Align>
EIGEN_DEVICE_FUNC constexpr void conditional_aligned_delete(T* ptr, std::size_t size) {
  destruct_elements_of_array<T>(ptr, size);
  conditional_aligned_free<Align>(ptr);
}

template <typename T, bool Align>
EIGEN_DEVICE_FUNC constexpr T* conditional_aligned_realloc_new(T* pts, std::size_t new_size, std::size_t old_size) {
...
template <typename T, bool Align>
EIGEN_DEVICE_FUNC constexpr T* conditional_aligned_new_auto(std::size_t size) {
...
template <typename T, bool Align>
EIGEN_DEVICE_FUNC constexpr T* conditional_aligned_realloc_new_auto(T* pts, std::size_t new_size,
                                                                    std::size_t old_size) {
  if (NumTraits<T>::RequireInitialization) {
...
template <typename T, bool Align>
EIGEN_DEVICE_FUNC constexpr void conditional_aligned_delete_auto(T* ptr, std::size_t size) {
...
struct smart_copy_helper<T, true> {
  EIGEN_DEVICE_FUNC static constexpr void run(const T* start, const T* end, T* target) {
...
struct smart_memmove_helper<T, true> {
  static constexpr void run(const T* start, const T* end, T* target) {
    std::intptr_t size = std::intptr_t(end) - std::intptr_t(start);
    if (size == 0) return;
    eigen_internal_assert(start != 0 && end != 0 && target != 0);
...
constexpr bool cpuid_is_vendor(int abcd[4], const int vendor[3]) {
