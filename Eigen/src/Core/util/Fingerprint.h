...
struct Fingerprint128 {
  ...
  // Fingerprint of a single byte range. Equivalent to one append() on a
  // Fingerprint128Builder; use the builder directly to span several ranges.
  static Fingerprint128 digest(const void* data, std::size_t bytes);
...
inline Fingerprint128 Fingerprint128::digest(const void* data, std::size_t bytes) {
