...
// A minimal stand-in for a by-value contraction sub-mapper: operator() returns
// by value (so it takes the no-direct-access dispatch) and packets advance the
// first index (the contiguous direction), exactly like a TensorContractionSub-
// Mapper over a contiguous LHS.
struct ByValueLhsMapper {
  const float* data;
...
// Its RHS counterpart: a by-value mapper whose LinearMapper advances depth (the
// contiguous direction), like a contraction RHS sub-mapper.
struct ByValueRhsMapper {
  struct LinearMapper {
...
template <bool PanelMode>
static void fill_lhs_ref(VectorXf& ref, const MatrixXf& V, Index rows, Index depth, Index dst_stride,
                         Index dst_offset) {
  const Index MR = internal::kSmeMr;
...
template <bool PanelMode>
static void fill_rhs_ref(VectorXf& ref, const MatrixXf& V, Index cols, Index depth, Index dst_stride,
                         Index dst_offset) {
  const Index NR = internal::kSmeNr;
  ref.setConstant(kPackSentinel);
  for (Index j = 0; j < cols; j += NR) {
...
// By-value LHS mapper (contraction-style): contiguous in the row direction.
template <bool PanelMode>
static void verify_fallback_lhs_byvalue(Index rows, Index depth) {
  MatrixXf V = MatrixXf::Random(rows, depth);
  ByValueLhsMapper mapper{V.data(), V.outerStride()};
  ...
  // The by-value mapper is storage-order agnostic and the fallback it takes is
  // too; instantiate the ColMajor specialization.
  internal::gemm_pack_lhs<float, Index, ByValueLhsMapper, internal::kSmeMr, 1,
                          typename internal::packet_traits<float>::type, ColMajor, false, PanelMode>
      pack;
  pack(packed.data(), mapper, depth, rows, dst_stride, dst_offset);
  VERIFY_IS_EQUAL(packed, ref);
}
...
// By-value RHS mapper (contraction-style): contiguous in the depth direction.
template <bool PanelMode>
static void verify_fallback_rhs_byvalue(Index depth, Index cols) {
  MatrixXf V = MatrixXf::Random(depth, cols);
  ByValueRhsMapper mapper{V.data(), V.outerStride()};
  ...
  internal::gemm_pack_rhs<float, Index, ByValueRhsMapper, internal::kSmeNr, ColMajor, false, PanelMode> pack;
  pack(packed.data(), mapper, depth, cols, dst_stride, dst_offset);
  VERIFY_IS_EQUAL(packed, ref);
}
...
static void test_mapper_fallback() {
  ...
  for (int n : widths) {
    for (int d : depths) {
      ...
      verify_fallback_lhs_byvalue<false>(n, d);
      verify_fallback_lhs_byvalue<true>(n, d);
      verify_fallback_rhs_byvalue<false>(d, n);
      verify_fallback_rhs_byvalue<true>(d, n);
    }
  }
}
...
EIGEN_DECLARE_TEST(product_sme) {
  ...
  // Raw packed-buffer checks: pin the SYMM packers and the mapper-based packing
  // fallback against a scalar reference, independently of the gebp_kernel.
  CALL_SUBTEST_1(test_symm_pack());
  CALL_SUBTEST_1(test_mapper_fallback());
}
