...
class LU {
 ...
 private:
  ...
  DeviceMatrix<Scalar> solve_impl(int64_t nrhs, int64_t ldb, TransposeMode mode, CopyRhs&& copy_rhs) const {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgetrs(ctx_.cusolver_, ctx_.params_.p, trans, n_, nrhs, dtype,
                                          d_lu_.get(), lda_, static_cast<const int64_t*>(d_ipiv_.get()), dtype, d_x_ptr,
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgetrf_bufferSize(ctx_.cusolver_, ctx_.params_.p, n_,
                                                     n_, dtype, d_lu_.get(), lda_, dtype,
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgetrf(
        ctx_.cusolver_, ctx_.params_.p, n_, n_, dtype, d_lu_.get(), lda_,
        static_cast<int64_t*>(d_ipiv_.get()), dtype, ctx_.scratch_workspace(), dev_ws_bytes,
