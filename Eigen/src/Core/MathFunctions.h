...
constexpr inline uint32_t log2(uint32_t x) {
  ...
  EIGEN_ALIGN_MAX const uint8_t table[32] = {
...
constexpr inline uint64_t log2(uint64_t x) {

  EIGEN_ALIGN_MAX const uint8_t table[64] = {
  ...
  x |= x >> 16;
  x |= x >> 32;
  return table[(((x - (x >> 1)) * 0x07EDD5E59A4E28C2)) >> 58];
}
