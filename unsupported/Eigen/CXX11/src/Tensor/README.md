...
Sometimes, conventional tensor expressions can be difficult to read. To write 
clearer expressions, one can use an 
[indexed expression](https://en.wikipedia.org/wiki/Index_notation), which is 
an extension of the [Einstein notation](https://en.wikipedia.org/wiki/Einstein_notation).
...
Because of the shuffling and chipping, it is difficult to follow which 
dimensions are contracted or added. Even worse, if the expression is wrong, 
we get no compilation error but an assertion failure at runtime (a crash).

Using `IndexedTensor`'s, the above expression can be equivalently rewritten as
...
`using namespace Eigen::tensor_indices;` or one can defined his own indices 
with `TensorIndex<some_letter_or_number> some_index_name;`.
...
By definition, an indexed expression does the same computation as if it was 
...
Let's brake down all possible features of the implemented index notation. 
Note for convenience we only use `TensorFixedSize` here. But the very same 
operations also work for dynamic sized `Tensor`'s.
...
In future the assignemnt of scalars to indexed tensors should also be 
implemented:

    Eigen::TensorFixedSize<float, Eigen::Sizes<3, 3>> A;
    A.setZero();
    using namespace Eigen::tensor_indices;
    A(i, 1) = 2.0f; // second column should be 2
    A(0, j) = 3.0f; // first row should be 3
    A(i, i) = 4.0f; // diagonals should be 4
...
#### Assignment to a `TensorMap`

All the shown features also work with tensor maps, e.g., we can assign a shuffled expression to a `TensorMap'

    Eigen::TensorFixedSize<float, Eigen::Sizes<2, 3, 4, 1>> A;
    A.setRandom();

    Eigen::TensorFixedSize<float, Eigen::Sizes<3, 1, 4, 2>> B;
    Eigen::TensorMap<decltype(B)> B_map(B.data(), B.dimensions());
    using namespace Eigen::tensor_indices;
    B_map(i, j, k, l) = A(l, i, k, j);
