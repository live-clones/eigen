...
struct fixed_size_tensor_index_linearization_helper {
  ...
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr Index run(array<Index, NumIndices> const& indices,
...
struct fixed_size_tensor_index_linearization_helper<Index, NumIndices, 0, RowMajor> {
  template <typename Dimensions>
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr Index run(array<Index, NumIndices> const&, const Dimensions&) {
    return 0;
  }
};
...
struct fixed_size_tensor_index_extraction_helper {
  template <typename Dimensions>
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr Index run(const Index index, const Dimensions& dimensions) {
...
struct fixed_size_tensor_index_extraction_helper<Index, 0> {
  template <typename Dimensions>
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr Index run(const Index, const Dimensions&) {
    return 0;
  }
};
...
struct Sizes {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::ptrdiff_t rank() const { return Base::count; }

  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::ptrdiff_t TotalSize() {
    return internal::arg_prod(Indices...);
  }

  EIGEN_DEVICE_FUNC Sizes() {}
  template <typename DenseIndex>
  explicit EIGEN_DEVICE_FUNC constexpr Sizes(const array<DenseIndex, Base::count>& /*indices*/) {
    // todo: add assertion
  }
  template <typename... DenseIndex>
  EIGEN_DEVICE_FUNC Sizes(DenseIndex...) {}
  explicit EIGEN_DEVICE_FUNC constexpr Sizes(std::initializer_list<std::ptrdiff_t> /*l*/) {
    // todo: add assertion
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::ptrdiff_t operator[](const std::ptrdiff_t index) const {
    return internal::fixed_size_tensor_index_extraction_helper<std::ptrdiff_t, Base::count>::run(index, t);
  }

  template <typename DenseIndex>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr ptrdiff_t IndexOfColMajor(
      const array<DenseIndex, Base::count>& indices) const {
    return internal::fixed_size_tensor_index_linearization_helper<DenseIndex, Base::count, Base::count, false>::run(
        indices, t);
  }
  template <typename DenseIndex>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr ptrdiff_t IndexOfRowMajor(
      const array<DenseIndex, Base::count>& indices) const {
...
namespace internal {
template <typename std::ptrdiff_t... Indices>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::ptrdiff_t array_prod(const Sizes<Indices...>&) {
...
struct DSizes : array<DenseIndex, NumDims> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr Index rank() const { return NumDims; }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr DenseIndex TotalSize() const {
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::ptrdiff_t array_get(const Sizes<Indices...>&) {
...
struct sizes_match_below_dim {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr bool run(Dims1&, Dims2&) { return false; }
};
template <typename Dims1, typename Dims2, ptrdiff_t n>
struct sizes_match_below_dim<Dims1, Dims2, n, n> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr bool run(Dims1& dims1, Dims2& dims2) {
...
struct sizes_match_below_dim<Dims1, Dims2, 0, 0> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr bool run(Dims1&, Dims2&) { return true; }
};

}  // end namespace internal

template <typename Dims1, typename Dims2>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE constexpr bool dimensions_match(Dims1 dims1, Dims2 dims2) {
