...
void test_multithread_contraction() {
  ...
  // Contractions accumulate O(contract_size) float rounding error; 1024 * eps of absolute slack covers it before
  // falling back to the relative check.
  for (ptrdiff_t i = 0; i < t_result.size(); i++) {
    VERIFY(&t_result.data()[i] != &m_result.data()[i]);
    if (fabsf(t_result(i) - m_result(i)) < 1024 * NumTraits<float>::epsilon()) {
      continue;
    }
    if (Eigen::internal::isApprox(t_result(i), m_result(i), 1024 * NumTraits<float>::epsilon())) {
      continue;
    }
...
void test_multithread_gemv_transpose() {
  for (int M : {17, 64, 129}) {
    for (int N : {16, 33, 100}) {
      ...
      for (int j = 0; j < N; ++j) {
        VERIFY(internal::isApprox(t_result(j), m_result(j), 1024 * NumTraits<float>::epsilon()));
      }
    }
  }
}
...
void test_contraction_corner_cases() {
  ...
  for (ptrdiff_t i = 0; i < t_result.size(); i++) {
    ...
    if (fabsf(t_result.data()[i] - m_result.data()[i]) >= 1024 * NumTraits<float>::epsilon()) {
