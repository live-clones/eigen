...
class LU {
 ...
 private:
  ...
  DeviceMatrix<Scalar> solve_impl(int64_t nrhs, int64_t ldb, TransposeMode mode, CopyRhs&& copy_rhs) const {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgetrs(handle_, params_.p, trans, n_, nrhs, dtype, d_lu_.get(),
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgetrf_bufferSize(handle_, params_.p, n_,
    ...
    EIGEN_CUSOLVER_CHECK(
        cusolverDnXgetrf(handle_, params_.p, n_, n_, dtype, d_lu_.get(),
