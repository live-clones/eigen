...
public:
  ...
  template<int LoadMode, typename PacketType>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
  PacketType partialPacketByOuterInner(Index outer, Index inner, Index n, Index offset) const
  { return Base::template partialPacket<LoadMode,PacketType>(IsRowMajor ? outer : inner, IsRowMajor ? inner : outer, n, offset); }
