...
class BlockImpl<XprType, BlockRows, BlockCols, true, Sparse>
 ...
 protected:
  ...
  using Base = SparseCompressedBase<BlockType>;
  using Base::convert_index;

 public:
  EIGEN_SPARSE_PUBLIC_INTERFACE(BlockType)

  constexpr BlockImpl(XprType& xpr, Index i) : m_matrix(xpr), m_outerStart(convert_index(i)), m_outerSize(OuterSize) {}

  constexpr BlockImpl(XprType& xpr, Index startRow, Index startCol, Index blockRows, Index blockCols)
      : m_matrix(xpr),
        m_outerStart(convert_index(IsRowMajor ? startRow : startCol)),
        m_outerSize(convert_index(IsRowMajor ? blockRows : blockCols)) {}

  EIGEN_STRONG_INLINE constexpr Index rows() const { return IsRowMajor ? m_outerSize.value() : m_matrix.rows(); }
  EIGEN_STRONG_INLINE constexpr Index cols() const { return IsRowMajor ? m_matrix.cols() : m_outerSize.value(); }
 ...
 protected:
  constexpr Index nonZerosImpl(std::true_type) const {
  ...
  constexpr Index nonZerosImpl(std::false_type) const {
 ...
 public:
  ...
  constexpr const XprType& nestedExpression() const { return m_matrix; }
  constexpr XprType& nestedExpression() { return m_matrix; }
  constexpr Index startRow() const { return IsRowMajor ? m_outerStart : 0; }
  constexpr Index startCol() const { return IsRowMajor ? 0 : m_outerStart; }
  constexpr Index blockRows() const { return IsRowMajor ? m_outerSize.value() : m_matrix.rows(); }
  constexpr Index blockCols() const { return IsRowMajor ? m_matrix.cols() : m_outerSize.value(); }

  constexpr const Scalar* valuePtr() const { return m_matrix.valuePtr(); }
  constexpr Scalar* valuePtr() { return m_matrix.valuePtr(); }

  constexpr const StorageIndex* innerIndexPtr() const { return m_matrix.innerIndexPtr(); }
  constexpr StorageIndex* innerIndexPtr() { return m_matrix.innerIndexPtr(); }

  constexpr const StorageIndex* outerIndexPtr() const {
    const StorageIndex* p = m_matrix.outerIndexPtr();
    return p ? p + m_outerStart : 0;
  }
  constexpr StorageIndex* outerIndexPtr() {
    StorageIndex* p = m_matrix.outerIndexPtr();
    return p ? p + m_outerStart : 0;
  }

  constexpr const StorageIndex* innerNonZeroPtr() const {
    const StorageIndex* p = m_matrix.innerNonZeroPtr();
    return p ? p + m_outerStart : 0;
  }
  constexpr StorageIndex* innerNonZeroPtr() {
    StorageIndex* p = m_matrix.innerNonZeroPtr();
    return p ? p + m_outerStart : 0;
  }

  constexpr bool isCompressed() const { return m_matrix.innerNonZeroPtr() == 0; }
 ...
 protected:
  ...
  template <typename T>
  constexpr BlockImpl& operator=(const T&) {
...
class sparse_matrix_block_impl : public SparseCompressedBase<Block<SparseMatrixType, BlockRows, BlockCols, true> > {
  using BlockType = Block<SparseMatrixType, BlockRows, BlockCols, true>;
  using Base = SparseCompressedBase<Block<SparseMatrixType, BlockRows, BlockCols, true>>;
  using Base::convert_index;
 ...
 public:
  constexpr sparse_matrix_block_impl(SparseMatrixType& xpr, Index i)
      : m_matrix(xpr), m_outerStart(convert_index(i)), m_outerSize(OuterSize) {}

  constexpr sparse_matrix_block_impl(SparseMatrixType& xpr, Index startRow, Index startCol, Index blockRows,
                                     Index blockCols)
      : m_matrix(xpr),
        m_outerStart(convert_index(IsRowMajor ? startRow : startCol)),
        m_outerSize(convert_index(IsRowMajor ? blockRows : blockCols)) {}
  ...
  constexpr BlockType& operator=(const SparseMatrixBase<OtherDerived>& other) {
    using NestedMatrixType_ = internal::remove_all_t<typename SparseMatrixType::Nested>;
  ...
  constexpr BlockType& operator=(const BlockType& other) { return operator= <BlockType>(other); }
  ...
  constexpr const StorageIndex* outerIndexPtr() const { return m_matrix.outerIndexPtr() + m_outerStart; }
  constexpr StorageIndex* outerIndexPtr() { return m_matrix.outerIndexPtr() + m_outerStart; }

  constexpr const StorageIndex* innerNonZeroPtr() const {
    return isCompressed() ? 0 : (m_matrix.innerNonZeroPtr() + m_outerStart);
  }
  constexpr StorageIndex* innerNonZeroPtr() { return isCompressed() ? 0 : (m_matrix.innerNonZeroPtr() + m_outerStart); }
  ...
  constexpr const Scalar& lastCoeff() const {
  ...
  constexpr const SparseMatrixType& nestedExpression() const { return m_matrix; }
  constexpr SparseMatrixType& nestedExpression() { return m_matrix; }
  constexpr Index startRow() const { return IsRowMajor ? m_outerStart : 0; }
  constexpr Index startCol() const { return IsRowMajor ? 0 : m_outerStart; }
  constexpr Index blockRows() const { return IsRowMajor ? m_outerSize.value() : m_matrix.rows(); }
  constexpr Index blockCols() const { return IsRowMajor ? m_matrix.cols() : m_outerSize.value(); }
...
class BlockImpl<SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true, Sparse>
 ...
 public:
  ...
  using Base = internal::sparse_matrix_block_impl<SparseMatrixType, BlockRows, BlockCols>;
  constexpr BlockImpl(SparseMatrixType& xpr, Index i) : Base(xpr, i) {}

  constexpr BlockImpl(SparseMatrixType& xpr, Index startRow, Index startCol, Index blockRows, Index blockCols)
...
class BlockImpl<const SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true, Sparse>
 ...
 public:
  using StorageIndex = StorageIndex_;
  using SparseMatrixType = const SparseMatrix<Scalar_, Options_, StorageIndex_>;
  using Base = internal::sparse_matrix_block_impl<SparseMatrixType, BlockRows, BlockCols>;
  constexpr BlockImpl(SparseMatrixType& xpr, Index i) : Base(xpr, i) {}
 ...
 private:
  ...
  constexpr BlockImpl(const SparseMatrixBase<Derived>& xpr, Index i);
  template <typename Derived>
  constexpr BlockImpl(const SparseMatrixBase<Derived>& xpr);
};
...
class BlockImpl<XprType, BlockRows, BlockCols, InnerPanel, Sparse>
  ...
  using Base = SparseMatrixBase<BlockType>;
  using Base::convert_index;
 ...
 public:
  ...
  using MatrixTypeNested_ = internal::remove_all_t<typename XprType::Nested>;

  /** Column or Row constructor
   */
  constexpr BlockImpl(XprType& xpr, Index i)
      : m_matrix(xpr),
  ...
  constexpr Index rows() const { return m_blockRows.value(); }
  constexpr Index cols() const { return m_blockCols.value(); }
  ...
  constexpr const XprType& nestedExpression() const { return m_matrix; }
  constexpr XprType& nestedExpression() { return m_matrix; }
  constexpr Index startRow() const { return m_startRow.value(); }
  constexpr Index startCol() const { return m_startCol.value(); }
  constexpr Index blockRows() const { return m_blockRows.value(); }
  constexpr Index blockCols() const { return m_blockCols.value(); }

 protected:
  friend struct internal::unary_evaluator<Block<XprType, BlockRows, BlockCols, InnerPanel>, internal::IteratorBased,
                                          Scalar>;

  constexpr Index nonZeros() const { return Dynamic; }
...
struct unary_evaluator<Block<ArgType, BlockRows, BlockCols, InnerPanel>, IteratorBased>
 ...
 public:
  ...
  using InnerIterator = std::conditional_t<OuterVector, OuterVectorInnerIterator, InnerVectorInnerIterator>;

  explicit constexpr unary_evaluator(const XprType& op) : m_argImpl(op.nestedExpression()), m_block(op) {}

  constexpr Index nonZerosEstimate() const {
...
class unary_evaluator<Block<ArgType, BlockRows, BlockCols, InnerPanel>, IteratorBased>::InnerVectorInnerIterator
 ...
 public:
  EIGEN_STRONG_INLINE constexpr InnerVectorInnerIterator(const unary_evaluator& aEval, Index outer)
  ...
  constexpr StorageIndex index() const {
  ...
  constexpr Index row() const { return EvalIterator::row() - m_block.startRow(); }
  constexpr Index col() const { return EvalIterator::col() - m_block.startCol(); }

  constexpr operator bool() const { return EvalIterator::operator bool() && EvalIterator::index() < m_end; }
};
...
class unary_evaluator<Block<ArgType, BlockRows, BlockCols, InnerPanel>, IteratorBased>::OuterVectorInnerIterator {
 ...
 public:
  EIGEN_STRONG_INLINE constexpr OuterVectorInnerIterator(const unary_evaluator& aEval, Index outer)
      : m_eval(aEval),
  ...
  constexpr Index outer() const { return 0; }
  constexpr Index row() const { return XprIsRowMajor ? 0 : index(); }
  constexpr Index col() const { return XprIsRowMajor ? index() : 0; }

  constexpr Scalar value() const { return m_it.value(); }
  constexpr Scalar& valueRef() { return m_it.valueRef(); }

  constexpr OuterVectorInnerIterator& operator++() {
  ...
  constexpr operator bool() const { return m_outerPos < m_end; }
};
...
struct unary_evaluator<Block<SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true>, IteratorBased>
  ...
  using XprType = Block<SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true>;
  using Base = evaluator<SparseCompressedBase<XprType>>;
  explicit constexpr unary_evaluator(const XprType& xpr) : Base(xpr) {}
};
...
struct unary_evaluator<Block<const SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true>,
  ...
  using XprType = Block<const SparseMatrix<Scalar_, Options_, StorageIndex_>, BlockRows, BlockCols, true>;
  using Base = evaluator<SparseCompressedBase<XprType>>;
  explicit constexpr unary_evaluator(const XprType& xpr) : Base(xpr) {}
};
