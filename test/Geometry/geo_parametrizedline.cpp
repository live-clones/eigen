...
EIGEN_DECLARE_TEST(geo_parametrizedline) {
  for (int i = 0; i < g_repeat; i++) {
    ...
    parametrizedline_alignment<float>();
    parametrizedline(ParametrizedLine<double, 4>());
    parametrizedline_alignment<double>();
    parametrizedline(ParametrizedLine<std::complex<double>, 5>());
  }
}
