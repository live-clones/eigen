...
class SparseMatrixBase : public EigenBase<Derived> {
 public:
  ...
  constexpr Derived& const_cast_derived() const { return *static_cast<Derived*>(const_cast<SparseMatrixBase*>(this)); }
...
#undef EIGEN_DOC_BLOCK_ADDONS_INNER_PANEL_IF
  ...
  constexpr Index rows() const { return derived().rows(); }
  /** \returns the number of columns. \sa rows() */
  constexpr Index cols() const { return derived().cols(); }
  /** \returns the number of coefficients, which is \a rows()*cols().
   * \sa rows(), cols(). */
  constexpr Index size() const { return rows() * cols(); }
  /** \returns true if either the number of rows or the number of columns is equal to 1.
   * In other words, this function returns
   * \code rows()==1 || cols()==1 \endcode
   * \sa rows(), cols(), IsVectorAtCompileTime. */
  constexpr bool isVector() const { return rows() == 1 || cols() == 1; }
  ...
  constexpr SparseMatrixBase() : m_isRValue(false) { /* TODO: validate traits flags. */
  }
 ...
 protected:
  template <typename OtherDerived>
  constexpr Derived& assign(const OtherDerived& other);

  template <typename OtherDerived>
  constexpr void assignGeneric(const OtherDerived& other);
 ...
 public:
  ...
  template <int Mode>
  constexpr const TriangularView<const Derived, Mode> triangularView() const;
  ...
  template <unsigned int UpLo>
  constexpr typename ConstSelfAdjointViewReturnType<UpLo>::Type selfadjointView() const;
  template <unsigned int UpLo>
   ...
   */
  constexpr const typename internal::eval<Derived>::type eval() const {
    return typename internal::eval<Derived>::type(derived());
  }

  constexpr Scalar sum() const;

  constexpr const SparseView<Derived> pruned(const Scalar& reference = Scalar(0),
                                             const RealScalar& epsilon = NumTraits<Scalar>::dummy_precision()) const;

 protected:
  bool m_isRValue;

  static constexpr StorageIndex convert_index(const Index idx) { return internal::convert_index<StorageIndex>(idx); }
