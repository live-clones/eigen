...
class SVD {
 public:
  ...
  DeviceMatrix<Scalar> d_matrixU() const {
    ...
    if (n_ > 0 && vtrows_stored > 0) {
      ...
      EIGEN_CUBLAS_CHECK(internal::cublasXgeam(
          solver_ctx_.cublasHandle(), CUBLAS_OP_C, CUBLAS_OP_N, internal::to_blas_int(n_),
  ...
  DeviceMatrix<Scalar> d_matrixVT() const {
    ...
    if (ucols > 0 && m_ > 0) {
      Scalar alpha_one(1), beta_zero(0);
      EIGEN_CUBLAS_CHECK(
          internal::cublasXgeam(solver_ctx_.cublasHandle(), CUBLAS_OP_C, CUBLAS_OP_N, internal::to_blas_int(ucols),
 ...
 private:
  ...
  void transpose_into_input(const DeviceMatrix<Scalar>& d_A) {
    ...
    Scalar alpha_one(1), beta_zero(0);
    EIGEN_CUBLAS_CHECK(internal::cublasXgeam(
        solver_ctx_.cublasHandle(), CUBLAS_OP_C, CUBLAS_OP_N, internal::to_blas_int(m_), internal::to_blas_int(n_),
  ...
  void apply_pinv(const Scalar* B_dev, Index kk, Index nrhs, Scalar* X_dev) const {
    ...
    EIGEN_CUBLAS_CHECK(internal::cublasXdgmm(
        solver_ctx_.cublasHandle(), CUBLAS_SIDE_LEFT, internal::to_blas_int(kk), internal::to_blas_int(nrhs), tmp_dev,
