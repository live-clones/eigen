...
//
// Unlike Eigen's CPU LLT<MatrixType>, LLT keeps the factored Cholesky
...
//
// Usage:
//   LLT<double> llt(A);              // upload A, potrf, L stays on device
//   if (llt.info() != Success) { ... }
...
class LLT {
 public:
  ...
  LLT& compute(const EigenBase<InputType>& A) {
    ...
    EIGEN_CUDA_RUNTIME_CHECK(
        cudaMemcpyAsync(d_factor_.ptr, mat.data(), factorBytes(), cudaMemcpyHostToDevice, ctx_.stream_));
  ...
  LLT& compute(const DeviceMatrix<Scalar>& d_A) {
    ...
    allocate_factor_storage();
    EIGEN_CUDA_RUNTIME_CHECK(
        cudaMemcpyAsync(d_factor_.ptr, d_A.data(), factorBytes(), cudaMemcpyDeviceToDevice, ctx_.stream_));
  ...
  PlainMatrix solve(const MatrixBase<Rhs>& B) const {
    ...
    DeviceMatrix<Scalar> d_X = solve_impl(nrhs, ldb, [&](Scalar* d_x_ptr) {
      EIGEN_CUDA_RUNTIME_CHECK(
          cudaMemcpyAsync(d_x_ptr, rhs.data(), rhsBytes(nrhs, ldb), cudaMemcpyHostToDevice, ctx_.stream_));
    });
  ...
  DeviceMatrix<Scalar> solve(const DeviceMatrix<Scalar>& d_B) const {
    ...
    return solve_impl(nrhs, ldb, [&](Scalar* d_x_ptr) {
      EIGEN_CUDA_RUNTIME_CHECK(
          cudaMemcpyAsync(d_x_ptr, d_B.data(), rhsBytes(nrhs, ldb), cudaMemcpyDeviceToDevice, ctx_.stream_));
    });
  }
 ...
 private:
  ...
  DeviceMatrix<Scalar> solve_impl(int64_t nrhs, int64_t ldb, CopyRhs&& copy_rhs) const {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXpotrs(ctx_.cusolver_, ctx_.params_.p, uplo, static_cast<int64_t>(n_), nrhs, dtype,
                                          d_factor_.ptr, lda_, dtype, d_x_ptr, ldb, ctx_.scratch_info()));
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXpotrf_bufferSize(ctx_.cusolver_, ctx_.params_.p, uplo, static_cast<int64_t>(n_),
                                                     dtype, d_factor_.ptr, lda_, dtype, &dev_ws_bytes,
                                                     &host_ws_bytes));

    ctx_.ensure_scratch(dev_ws_bytes);
    ctx_.h_workspace_.resize(host_ws_bytes);

    EIGEN_CUSOLVER_CHECK(cusolverDnXpotrf(ctx_.cusolver_, ctx_.params_.p, uplo, static_cast<int64_t>(n_), dtype,
                                          d_factor_.ptr, lda_, dtype, ctx_.scratch_workspace(), dev_ws_bytes,
                                          host_ws_bytes > 0 ? ctx_.h_workspace_.data() : nullptr, host_ws_bytes,
                                          ctx_.scratch_info()));
