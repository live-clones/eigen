...
// Even 64-bit lanes hold real parts, odd lanes imaginary parts.
EIGEN_STRONG_INLINE svbool_t sve_cd_even_lanes() {
  return svcmpeq_n_u64(svptrue_b64(), svand_n_u64_x(svptrue_b64(), svindex_u64(0, 1), 1), 0);
}

template <>
EIGEN_STRONG_INLINE PacketXcd pset1<PacketXcd>(const std::complex<double>& from) {
  return PacketXcd(svsel_f64(sve_cd_even_lanes(), svdup_n_f64(numext::real(from)), svdup_n_f64(numext::imag(from))));
}
...
EIGEN_STRONG_INLINE PacketXcd ploaddup<PacketXcd>(const std::complex<double>* from) {
  const svuint64_t value = svlsr_n_u64_x(svptrue_b64(), svindex_u64(0, 1), 2);
  return PacketXcd(
...
EIGEN_STRONG_INLINE PacketXcd ploadquad<PacketXcd>(const std::complex<double>* from) {
  const svuint64_t value = svlsr_n_u64_x(svptrue_b64(), svindex_u64(0, 1), 3);
  return PacketXcd(
      svld1_gather_u64index_f64(svptrue_b64(), reinterpret_cast<const double*>(from), sve_cd_component_index(value)));
}
...
EIGEN_STRONG_INLINE std::complex<double> pfirst<PacketXcd>(const PacketXcd& a) {
  ...
  return std::complex<double>(svlastb_f64(svptrue_pat_b64(SV_VL1), a.v), svlastb_f64(svptrue_pat_b64(SV_VL2), a.v));
}

template <>
EIGEN_STRONG_INLINE PacketXcd pconj(const PacketXcd& a) {
  const svuint64_t mask = svdup_n_u64_z(svnot_b_z(svptrue_b64(), sve_cd_even_lanes()), numext::uint64_t(1) << 63);
...
EIGEN_STRONG_INLINE std::complex<double> predux<PacketXcd>(const PacketXcd& a) {
  const svbool_t even = sve_cd_even_lanes();
  return std::complex<double>(svaddv_f64(even, a.v), svaddv_f64(svnot_b_z(svptrue_b64(), even), a.v));
}
