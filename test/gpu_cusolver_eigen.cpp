...
void test_eigen_values(Index n) {
  ...
  RealScalar tol = RealScalar(2) * static_cast<RealScalar>(n) * NumTraits<Scalar>::epsilon() *
...
void test_eigen_values_only(Index n) {
  ...
  gpu::SelfAdjointEigenSolver<Scalar> gpu_es(A, gpu::SelfAdjointEigenSolver<Scalar>::EigenvaluesOnly);
  VERIFY_IS_EQUAL(gpu_es.info(), Success);
  auto W_gpu = gpu_es.eigenvalues();
