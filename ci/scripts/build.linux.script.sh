...
# resource use.
# EIGEN_CI_BUILD_JOBS can be set to limit parallelism (e.g. -j2 for sanitizer
# builds that need more memory per TU).
jobs_flag=""
if [[ -n "${EIGEN_CI_BUILD_JOBS}" ]]; then
  jobs_flag="-j${EIGEN_CI_BUILD_JOBS}"
fi
cmake --build . ${target} -- -k0 ${jobs_flag} || cmake --build . ${target} -- -k0 -j1
