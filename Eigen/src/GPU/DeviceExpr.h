...
// Lightweight expression types for gpu::DeviceMatrix operations.
//
...
//
// Expression types:
//   gpu::AdjointView<S>   — d_A.adjoint()  → marks ConjTrans for GEMM
//   gpu::TransposeView<S> — d_A.transpose() → marks Trans for GEMM
//   gpu::Scaled<Expr>     — alpha * expr    → carries scalar factor
...
namespace Eigen {
namespace internal {
// Forward declaration — specializations follow below, after the gpu:: class definitions.
...
// Returned by gpu::DeviceMatrix::adjoint(). Maps to cublasXgemm transA/B = C.
...
// Returned by gpu::DeviceMatrix::transpose(). Maps to cublasXgemm transA/B = T.
...
// ---- Scaled: alpha * expr ---------------------------------------------------
// Returned by operator*(Scalar, gpu::DeviceMatrix/View). Carries the scalar factor.
...
class GemmExpr {
 public:
  ...
  static_assert(std::is_same<Scalar, typename internal::device_expr_traits<Rhs>::scalar_type>::value,
                "gpu::DeviceMatrix GEMM: LHS and RHS must have the same scalar type");
 ...
 private:
  ...
  // assign to a gpu::DeviceMatrix immediately.
  const Lhs& lhs_;
  const Rhs& rhs_;
};
...
// Default: a gpu::DeviceMatrix is NoTrans.
template <typename T>
struct device_expr_traits {
...
struct device_expr_traits<gpu::DeviceMatrix<Scalar>> {
  using scalar_type = Scalar;
  static constexpr GpuOp op = GpuOp::NoTrans;
  static constexpr bool is_device_expr = true;
  static const gpu::DeviceMatrix<Scalar>& matrix(const gpu::DeviceMatrix<Scalar>& x) { return x; }
  static Scalar alpha(const gpu::DeviceMatrix<Scalar>&) { return Scalar(1); }
};

template <typename Scalar>
struct device_expr_traits<gpu::AdjointView<Scalar>> {
  using scalar_type = Scalar;
  static constexpr GpuOp op = GpuOp::ConjTrans;
  static constexpr bool is_device_expr = true;
  static const gpu::DeviceMatrix<Scalar>& matrix(const gpu::AdjointView<Scalar>& x) { return x.matrix(); }
  static Scalar alpha(const gpu::AdjointView<Scalar>&) { return Scalar(1); }
};

template <typename Scalar>
struct device_expr_traits<gpu::TransposeView<Scalar>> {
  using scalar_type = Scalar;
  static constexpr GpuOp op = GpuOp::Trans;
  static constexpr bool is_device_expr = true;
  static const gpu::DeviceMatrix<Scalar>& matrix(const gpu::TransposeView<Scalar>& x) { return x.matrix(); }
  static Scalar alpha(const gpu::TransposeView<Scalar>&) { return Scalar(1); }
};

template <typename Inner>
struct device_expr_traits<gpu::Scaled<Inner>> {
  using scalar_type = typename device_expr_traits<Inner>::scalar_type;
  static constexpr GpuOp op = device_expr_traits<Inner>::op;
  static constexpr bool is_device_expr = true;
  static const gpu::DeviceMatrix<scalar_type>& matrix(const gpu::Scaled<Inner>& x) {
    return device_expr_traits<Inner>::matrix(x.inner());
  }
  static scalar_type alpha(const gpu::Scaled<Inner>& x) {
