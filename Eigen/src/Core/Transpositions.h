...
class TranspositionsBase {
 ...
 public:
  ...
  constexpr Derived& operator=(const TranspositionsBase<OtherDerived>& other) {
    indices() = other.indices();
    return derived();
  }

  /** \returns the number of transpositions */
  EIGEN_DEVICE_FUNC constexpr Index size() const { return indices().size(); }
  ...
  EIGEN_DEVICE_FUNC constexpr const StorageIndex& coeff(Index i) const { return indices().coeff(i); }
  /** Direct access to the underlying index vector */
  constexpr StorageIndex& coeffRef(Index i) { return indices().coeffRef(i); }
  /** Direct access to the underlying index vector */
  constexpr const StorageIndex& operator()(Index i) const { return indices()(i); }
  /** Direct access to the underlying index vector */
  constexpr StorageIndex& operator()(Index i) { return indices()(i); }
  /** Direct access to the underlying index vector */
  constexpr const StorageIndex& operator[](Index i) const { return indices()(i); }
  /** Direct access to the underlying index vector */
  constexpr StorageIndex& operator[](Index i) { return indices()(i); }
  ...
  constexpr void resize(Index newSize) { indices().resize(newSize); }
  ...
  constexpr void applyForwardToRows(MatrixType& mat) const
  {
  ...
  template<typename MatrixType>
  constexpr void applyBackwardToRows(MatrixType& mat) const
  {
  ...
  */
  ...
  constexpr Transpose<TranspositionsBase> inverse() const { return Transpose<TranspositionsBase>(derived()); }

  /** \returns the transpose transformation */
  constexpr Transpose<TranspositionsBase> transpose() const { return Transpose<TranspositionsBase>(derived()); }
...
class Transpositions
 ...
 public:
  ...
  constexpr Transpositions() = default;

  /** Copy constructor. */
  template <typename OtherDerived>
  constexpr Transpositions(const TranspositionsBase<OtherDerived>& other) : m_indices(other.indices()) {}

  /** Generic constructor from expression of the transposition indices. */
  template <typename Other>
  explicit constexpr Transpositions(const MatrixBase<Other>& indices) : m_indices(indices) {}
   ...
   */
  constexpr Transpositions(Index size) : m_indices(size) {}
...
class Map<Transpositions<SizeAtCompileTime, MaxSizeAtCompileTime, StorageIndex_>, PacketAccess>
 ...
 public:
  ...
  explicit constexpr Map(const StorageIndex* indicesPtr) : m_indices(indicesPtr) {}

  constexpr Map(const StorageIndex* indicesPtr, Index size) : m_indices(indicesPtr, size) {}

  /** Copies the \a other transpositions into \c *this */
  template <typename OtherDerived>
  constexpr Map& operator=(const TranspositionsBase<OtherDerived>& other) {
    return Base::operator=(other);
  }
...
class TranspositionsWrapper : public TranspositionsBase<TranspositionsWrapper<IndicesType_> > {
 ...
 public:
  ...
  explicit constexpr TranspositionsWrapper(IndicesType& indices) : m_indices(indices) {}

  /** Copies the \a other transpositions into \c *this */
  template <typename OtherDerived>
  constexpr TranspositionsWrapper& operator=(const TranspositionsBase<OtherDerived>& other) {
    return Base::operator=(other);
  }
...
EIGEN_DEVICE_FUNC constexpr const Product<MatrixDerived, TranspositionsDerived, AliasFreeProduct> operator*(
...
EIGEN_DEVICE_FUNC constexpr const Product<TranspositionsDerived, MatrixDerived, AliasFreeProduct> operator*(
...
class Transpose<TranspositionsBase<TranspositionsDerived> > {
 ...
 public:
  constexpr explicit Transpose(const TranspositionType& t) : m_transpositions(t) {}
   ...
   */
  template <typename OtherDerived>
  friend constexpr const Product<OtherDerived, Transpose, AliasFreeProduct> operator*(
   ...
   */
  template <typename OtherDerived>
  constexpr const Product<Transpose, OtherDerived, AliasFreeProduct> operator*(
      const MatrixBase<OtherDerived>& matrix) const {
    return Product<Transpose, OtherDerived, AliasFreeProduct>(*this, matrix.derived());
  }

  EIGEN_DEVICE_FUNC constexpr const TranspositionType& nestedExpression() const { return m_transpositions; }
