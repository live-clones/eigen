...
#pragma omp section
          {
            dst.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template triangularView<Lower>() = tmp * rhs.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template selfadjointView<Lower>() + lhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2).transpose() * rhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2);
          }
#pragma omp section
          {
            dst.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) = lhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) * rhs.nestedExpression().block(1, 0, dimension / 2, dimension / 2).template selfadjointView<Lower>() + lhs.nestedExpression().block(0, 0, dimension / 2, dimension / 2).template selfadjointView<Upper>() * rhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2);
          }
#pragma omp section
          {
            dst.nestedExpression().block(0, 0, dimension / 2, dimension / 2).template triangularView<Upper>() = lhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2) * rhs.nestedExpression().block(dimension / 2 + 1, 0, dimension / 2, dimension / 2).transpose() + tmp2 * rhs.nestedExpression().block(0, 0, dimension / 2, dimension / 2).template selfadjointView<Upper>();
          }
        }
      }
    };
  ...
  public:
        ...
        : m_dimension(other.cols()), m_odd(other.cols() % 2)
    {
      ...
      EIGEN_STATIC_ASSERT(OtherDerived::RowsAtCompileTime == _Dimension && OtherDerived::RowsAtCompileTime == OtherDerived::ColsAtCompileTime,
    ...
    EIGEN_DEVICE_FUNC
        HermitianMatrix &
        operator=(const HermitianBase<OtherDerived> &other)
    {
...
#pragma omp section
            {
              this->nestedExpression().block(1, 0, m_dimension / 2, m_dimension / 2).template triangularView<Lower>() = tmp * nestedRhs.block(1, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Lower>() + nestedLhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2).transpose() * nestedRhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2);
            }
#pragma omp section
            {
              this->nestedExpression().block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) = nestedLhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) * nestedRhs.block(1, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Lower>() + nestedLhs.block(0, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Upper>() * nestedRhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2);
            }
#pragma omp section
            {
              this->nestedExpression().block(0, 0, m_dimension / 2, m_dimension / 2).template triangularView<Upper>() = nestedLhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2) * nestedRhs.block(m_dimension / 2 + 1, 0, m_dimension / 2, m_dimension / 2).transpose() + tmp2 * nestedRhs.block(0, 0, m_dimension / 2, m_dimension / 2).template selfadjointView<Upper>();
            }
          }
        }
        else if (_UpLo == Upper)
