...
void BDCSVD<MatrixType, Options>::copyUV(const HouseholderU& householderU, const HouseholderV& householderV,
  ...
  if (computeU())
  {
    Index Ucols = m_computeThinU ? m_diagSize.value() : rows();
    m_matrixU = MatrixX::Identity(rows(), Ucols);
    m_matrixU.topLeftCorner(m_diagSize.value(), m_diagSize.value()) = naiveV.template cast<Scalar>().topLeftCorner(m_diagSize.value(), m_diagSize.value());
    // FIXME the following conditionals involve temporary buffers
    if (m_useQrDecomp) m_matrixU.topLeftCorner(householderU.cols(), m_diagSize.value()).applyOnTheLeft(householderU);
    else m_matrixU.applyOnTheLeft(householderU);
  }
  if (computeV())
  {
    Index Vcols = m_computeThinV ? m_diagSize.value() : cols();
    m_matrixV = MatrixX::Identity(cols(), Vcols);
    m_matrixV.topLeftCorner(m_diagSize.value(), m_diagSize.value()) = naiveU.template cast<Scalar>().topLeftCorner(m_diagSize.value(), m_diagSize.value());
    // FIXME the following conditionals involve temporary buffers
    if (m_useQrDecomp) m_matrixV.topLeftCorner(householderV.cols(), m_diagSize.value()).applyOnTheLeft(householderV);
