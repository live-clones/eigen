...
template <typename T>
constexpr T atan_series_order(const T x, const T x_pow, const uint_t order_begin, const uint_t max_order) noexcept {
...
constexpr T atan_series_order(const T x, const T x_pow, const uint_t order, const uint_t max_order) noexcept {
  return (max_order == 1 ? T(GCEM_HALF_PI) - T(1) / x
          : order == 1   ? T(GCEM_HALF_PI) - T(1) / x + atan_series_order(x * x, pow(x, 3), order + 1, max_order)
                         :
...
template <typename T>
constexpr T atan_cf_recur(const T xx, const uint_t depth_begin, const uint_t max_depth) noexcept {
