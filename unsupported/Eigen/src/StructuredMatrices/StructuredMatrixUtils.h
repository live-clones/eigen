 ...
 *
 * Genuinely non-finite data (NaN or Inf in the symbol or the right-hand side)
 * must not reach this function: the transforms mix every input entry into every
 * output entry, so a single special value would contaminate the whole column
 * with NaN where the dense product only propagates it through the dot products
 * that touch it. The operators route such data to their direct kernels instead.
 */
