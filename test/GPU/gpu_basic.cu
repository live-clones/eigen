...
TEST(GPUBasicTest, Basic) {
  ...
  run_and_compare_to_gpu(coeff_wise<Vector3f>(), nthreads, in, out);
  run_and_compare_to_gpu(coeff_wise<Array44f>(), nthreads, in, out);
  ...
  //           constructor
  run_and_compare_to_gpu(replicate<Array4f>(), nthreads, in, out);
  run_and_compare_to_gpu(replicate<Array33f>(), nthreads, in, out);

  // HIP does not support new/delete on device.
  run_and_compare_to_gpu(alloc_new_delete<Vector3f>(), nthreads, in, out);
#endif

  run_and_compare_to_gpu(redux<Array4f>(), nthreads, in, out);
  run_and_compare_to_gpu(redux<Matrix3f>(), nthreads, in, out);

  run_and_compare_to_gpu(prod_test<Matrix3f, Matrix3f>(), nthreads, in, out);
  run_and_compare_to_gpu(prod_test<Matrix4f, Vector4f>(), nthreads, in, out);

  run_and_compare_to_gpu(diagonal<Matrix3f, Vector3f>(), nthreads, in, out);
  run_and_compare_to_gpu(diagonal<Matrix4f, Vector4f>(), nthreads, in, out);

  run_and_compare_to_gpu(matrix_inverse<Matrix2f>(), nthreads, in, out);
  run_and_compare_to_gpu(matrix_inverse<Matrix3f>(), nthreads, in, out);
  run_and_compare_to_gpu(matrix_inverse<Matrix4f>(), nthreads, in, out);

  run_and_compare_to_gpu(eigenvalues_direct<Matrix3f>(), nthreads, in, out);
  run_and_compare_to_gpu(eigenvalues_direct<Matrix2f>(), nthreads, in, out);

  // Test std::complex.
  run_and_compare_to_gpu(complex_operators<Vector3cf>(), nthreads, cfin, cfout);
  test_with_infs_nans(complex_sqrt<Vector3cf>(), nthreads, cfin, cfout);

  // numeric_limits
  test_with_infs_nans(numeric_limits_test<Vector3f>(), 1, in, out);

  // These tests require dynamic-sized matrix multiplication, which isn't currently
  // supported on GPU.

  //  run_and_compare_to_gpu(eigenvalues<Matrix4f>(), nthreads, in, out) ;
