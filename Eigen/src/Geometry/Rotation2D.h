...
class Rotation2D : public RotationBase<Rotation2D<Scalar_>, 2> {
 ...
 public:
  ...
  EIGEN_DEVICE_FUNC explicit constexpr Rotation2D(const Scalar& a) : m_angle(a) {}
  ...
  EIGEN_DEVICE_FUNC constexpr Scalar smallestPositiveAngle() const {
    Scalar tmp = numext::fmod(m_angle, Scalar(2 * EIGEN_PI));
    return tmp < Scalar(0) ? tmp + Scalar(2 * EIGEN_PI) : tmp;
  }

  /** \returns the rotation angle in [-pi,pi] */
  EIGEN_DEVICE_FUNC constexpr Scalar smallestAngle() const {
    Scalar tmp = numext::fmod(m_angle, Scalar(2 * EIGEN_PI));
  ...
  EIGEN_DEVICE_FUNC constexpr Rotation2D inverse() const { return Rotation2D(-m_angle); }

  /** Concatenates two rotations */
  EIGEN_DEVICE_FUNC constexpr Rotation2D operator*(const Rotation2D& other) const {
    return Rotation2D(m_angle + other.m_angle);
  }

  /** Concatenates two rotations */
  EIGEN_DEVICE_FUNC constexpr Rotation2D& operator*=(const Rotation2D& other) {
   ...
   */
  EIGEN_DEVICE_FUNC constexpr Rotation2D slerp(const Scalar& t, const Rotation2D& other) const {
  ...
  EIGEN_DEVICE_FUNC constexpr typename internal::cast_return_type<Rotation2D, Rotation2D<NewScalarType> >::type cast()
      const {
  ...
  EIGEN_DEVICE_FUNC constexpr explicit Rotation2D(const Rotation2D<OtherScalarType>& other) {
    m_angle = Scalar(other.angle());
  }

  EIGEN_DEVICE_FUNC static constexpr Rotation2D Identity() { return Rotation2D(0); }
