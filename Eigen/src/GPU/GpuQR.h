...
class QR {
 public:
  ...
  QR& compute(const DeviceMatrix<Scalar>& d_A) {
    ...
    EIGEN_CUDA_RUNTIME_CHECK(
        cudaMemcpyAsync(d_qr_.ptr, d_A.data(), mat_bytes, cudaMemcpyDeviceToDevice, ctx_.stream_));
  ...
  DeviceMatrix<Scalar> solve(const DeviceMatrix<Scalar>& d_B) const {
    ...
    EIGEN_CUDA_RUNTIME_CHECK(
        cudaMemcpyAsync(d_work.ptr, d_B.data(), b_bytes, cudaMemcpyDeviceToDevice, ctx_.stream_));
    ...
    if (m_ == n_) {
      ...
      DeviceMatrix<Scalar> result = DeviceMatrix<Scalar>::adopt(static_cast<Scalar*>(d_work.ptr), n_, static_cast<Index>(nrhs));
 ...
 private:
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgeqrf(ctx_.cusolver_, ctx_.params_.p, static_cast<int64_t>(m_),
                                          static_cast<int64_t>(n_), dtype, d_qr_.ptr, lda_, dtype, d_tau_.ptr, dtype,
                                          ctx_.scratch_workspace(), dev_ws,
  ...
  void apply_QH(void* d_B, int64_t ldb, int64_t nrhs) const {
    ...
    EIGEN_CUSOLVER_CHECK(internal::cusolverDnXormqr(ctx_.cusolver_, CUBLAS_SIDE_LEFT, trans, im, in, ik,
                                                    static_cast<const Scalar*>(d_qr_.ptr), ilda,
                                                    static_cast<const Scalar*>(d_tau_.ptr), static_cast<Scalar*>(d_B),
                                                    ildb, static_cast<Scalar*>(d_work.ptr), lwork,
                                                    ctx_.scratch_info()));
