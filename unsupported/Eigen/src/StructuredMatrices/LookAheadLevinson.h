...
typename D1::Scalar structured_tdot(const MatrixBase<D1>& a, const MatrixBase<D2>& b) {
  ...
  Scalar s(0);
  for (Index i = 0; i < a.size(); ++i) s += a.coeff(i) * b.coeff(i);
  return s;
}
...
class LookAheadLevinson {
 public:
  ...
  /** Constructs and factorizes from the Toeplitz matrix \a T. */
  explicit LookAheadLevinson(const Toeplitz<Scalar>& T)
  ...
  LookAheadLevinson& compute(const Toeplitz<Scalar>& T);
...
template <typename Scalar_>
LookAheadLevinson<Scalar_>& LookAheadLevinson<Scalar_>::compute(const Toeplitz<Scalar>& T) {
