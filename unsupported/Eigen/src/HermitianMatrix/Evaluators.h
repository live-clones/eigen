...
struct HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression, 0> {
  ...
  operator Scalar() const { return m_nested->coeff(m_row, m_col); }
...
struct evaluator<HermitianBase<Derived> > : evaluator_base<HermitianBase<Derived> > {
  ...
  EIGEN_DEVICE_FUNC evaluator(const HermitianType& m) : m_nested(&m.nestedExpression()), m_dimension(m.cols()) {}

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression> coeff(
      Index row, Index col) const {
    // Even dimension and upper triangular storage
    if (UpLo == Upper && !(m_dimension % 2)) {
      if (row <= col) {
        if (col < m_dimension / 2) {
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(m_dimension / 2 + col + 1, row, m_nested,
                                                                            true);
        } else {
        ...
        if (col <= row) {
          if (row < m_dimension / 2) {
            return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(m_dimension / 2 + row + 1, col, m_nested,
                                                                              false);
          } else {
            return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(col, row - m_dimension / 2, m_nested,
                                                                              false);
          }
        }
      }
    ...
    if (UpLo == Lower && !(m_dimension % 2)) {
      if (row < col) {
        if (row < m_dimension / 2) {
          ...
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(row - m_dimension / 2,
                                                                            col - m_dimension / 2, m_nested, false);
        }
      } else {
        if (col < m_dimension / 2) {
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(row + 1, col, m_nested, true);
        } else {
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(col - m_dimension / 2,
                                                                            row - m_dimension / 2, m_nested, true);
        }
      }
    }
    ...
    if (UpLo == Upper && (m_dimension % 2)) {
      if (row <= col) {
        if (col < (m_dimension - 1) / 2) {
          ...
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(row, col - (m_dimension - 1) / 2, m_nested,
                                                                            true);
        }
      } else {
        if (row < (m_dimension - 1) / 2) {
          ...
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(col, row - (m_dimension - 1) / 2, m_nested,
                                                                            false);
        }
      }
    }
    ...
    if (UpLo == Lower && (m_dimension % 2)) {
      if (row < col) {
        if (row < (m_dimension + 1) / 2) {
          ...
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(
              row - (m_dimension - 1) / 2 - 1, col - (m_dimension + 1) / 2 + 1, m_nested, false);
        }
      } else {
        if (col < (m_dimension + 1) / 2) {
          ...
          return HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression>(
              col - (m_dimension - 1) / 2 - 1, row - (m_dimension + 1) / 2 + 1, m_nested, true);
        }
      }
    }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE HermitianMatrixCoeffReturnHelper<Scalar, NestedExpression> coeffRef(Index row,
                                                                                                            Index col) {
    // Even dimension and upper triangular storage
    if (UpLo == Upper && !(m_dimension % 2)) {
      if (row <= col) {
