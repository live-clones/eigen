...
struct MemcpyTriggerForSlicing {
  EIGEN_DEVICE_FUNC constexpr MemcpyTriggerForSlicing(const Device& device)
...
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device> {
 ...
 protected:
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr Index srcCoeff(Index index) const { return srcCoeffImpl(index); }

  template <int ND = NumDims>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr std::enable_if_t<ND == 0, Index> srcCoeffImpl(Index /*index*/) const {
    return 0;
  }
...
struct TensorEvaluator<const TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr typename Storage::Type data() const { return nullptr; }
