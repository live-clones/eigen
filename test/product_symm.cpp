...
void make_stored_triangle(Index n, MatrixType& stored, MatrixType& full) {
  ...
  for (Index r = 0; r < n; ++r)
    for (Index c = 0; c <= r; ++c) stored(r, c) = full(r, c);
}
...
void check_symm_pack_lhs(Index kc) {
  ...
  const Scalar marker = Scalar(typename NumTraits<Scalar>::Real(-31415));
  Matrix<Scalar, Dynamic, 1> packed = Matrix<Scalar, Dynamic, 1>::Constant(kc * kc + 32, marker);
...
void check_symm_pack_rhs(Index n, Index rows, Index k2) {
  ...
  const Scalar marker = Scalar(typename NumTraits<Scalar>::Real(-31415));
  Matrix<Scalar, Dynamic, 1> packed = Matrix<Scalar, Dynamic, 1>::Constant(rows * n + 32, marker);
  Matrix<Scalar, Dynamic, 1> ref = packed;
...
void symm_packers_and_rowmajor_operands() {
  ...
  for (Index n : sizes) {
    for (Index m : {Index(1), Index(3), Index(17)}) {
