...
void test_kron_sparse_determinant_subnormal(const Scalar& phase) {
  ...
  // ARMv7 NEON reads subnormal operands as zero under no FPSCR control: the packet exponent bound then sees a zero
  // factor, skips the scale-up, and the elimination keeps only those digits/2 bits.
