...
class MatrixMarketIterator {
 ...
 public:
  ...
  constexpr MatrixMarketIterator& operator++() {
    m_matIsLoaded = false;
    m_hasrefX = false;
    m_hasRhs = false;
    Getnextvalidmatrix();
  ...
  constexpr operator bool() const { return m_isvalid; }
  ...
  constexpr std::string& matname() { return m_matname; }

  constexpr inline int sym() const { return m_sym; }

  constexpr bool hasRhs() const { return m_hasRhs; }
  constexpr bool hasrefX() const { return m_hasrefX; }
  constexpr bool isFolderValid() const { return bool(m_folder_id); }

 protected:
  constexpr inline bool Fileexists(std::string file) const {
  ...
  constexpr void Getnextvalidmatrix() {
    ...
    while ((m_curs_id = readdir(m_folder_id)) != NULL) {
      ...
      if (iscomplex) {
        if (std::is_same<Scalar, float>::value || std::is_same<Scalar, double>::value) continue;
      }
