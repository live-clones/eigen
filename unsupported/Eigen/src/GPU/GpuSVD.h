...
class SVD {
 public:
  ...
  SVD& compute(const DeviceMatrix<Scalar>& d_A, unsigned int options = ComputeThinU | ComputeThinV) {
    ...
    if (transposed_) {
      ...
      d_A_ = internal::DeviceBuffer(mat_bytes, solver_ctx_.stream_);
  ...
  SVD& compute(DeviceMatrix<Scalar>&& d_A, unsigned int options = ComputeThinU | ComputeThinV) {
    ...
    if (transposed_) {
      ...
      d_A_ = internal::DeviceBuffer::adopt(static_cast<void*>(d_A.release()), a_bytes, solver_ctx_.stream_);
    }
 ...
 private:
  ...
  void factorize() {
    ...
    internal::ensure_sized(d_S_, static_cast<size_t>(k) * sizeof(RealScalar), solver_ctx_.stream_);
  ...
  void build_diag(Index kk, RealScalar lambda) const {
    ...
    internal::ensure_sized(d_D_, d_bytes, solver_ctx_.stream_);
    EIGEN_CUDA_RUNTIME_CHECK(
