...
- **New SIMD intrinsics need a `test/packetmath.cpp` entry** (or `unsupported/test/special_packetmath.cpp` for special functions), and a specialization in every architecture backend that supports the type — `Default/` provides scalar fallbacks.
- **Compile-failure tests** live in `failtest/`. If you're tightening a static check (a new `EIGEN_STATIC_ASSERT`, a deleted overload), add an `_ok` / `_ko` pair so CI catches regressions in the diagnostic.
