...
# Collect packages to install, then do a single apt-get update + install.
pkgs=""

# Core build tools (skip if already present, e.g. in a pre-built CI image).
if ! command -v cmake >/dev/null 2>&1 || ! command -v ninja >/dev/null 2>&1; then
  pkgs="ninja-build cmake git"
fi
