...
class LU {
 public:
  ...
  LU& compute(DeviceMatrix<Scalar>&& d_A) {
    ...
    lda_ = static_cast<int64_t>(d_A.rows());
    d_A.waitReady(solver_ctx_.stream());
    d_lu_ = internal::DeviceBuffer::adopt(static_cast<void*>(d_A.release()), matrixBytes(), solver_ctx_.stream());
  ...
  PlainMatrix solve(const MatrixBase<Rhs>& B, GpuOp op = GpuOp::NoTrans) const {
    ...
    internal::DeviceBuffer d_x(matrixBytes(nrhs, ldb), solver_ctx_.stream());
    EIGEN_CUDA_RUNTIME_CHECK(
  ...
  DeviceMatrix<Scalar> solve(DeviceMatrix<Scalar>&& d_B, GpuOp op = GpuOp::NoTrans) const {
    ...
    internal::DeviceBuffer d_x =
        internal::DeviceBuffer::adopt(static_cast<void*>(d_B.release()), matrixBytes(nrhs, ldb), solver_ctx_.stream());
    return solve_impl(nrhs, ldb, op, std::move(d_x));
  }
 ...
 private:
  ...
  void allocate_lu_storage() { internal::ensure_sized(d_lu_, matrixBytes(), solver_ctx_.stream()); }
  ...
  void factorize() {
    ...
    internal::ensure_sized(d_ipiv_, ipiv_bytes, solver_ctx_.stream());
