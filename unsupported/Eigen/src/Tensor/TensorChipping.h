...
struct TensorEvaluator<const TensorChippingOp<DimId, ArgType>, Device> {
 ...
 protected:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr Index srcCoeffInnerChipping(Index index) const {
    // m_stride is equal to 1, so let's avoid the integer division.
    eigen_assert(m_stride == 1);
    return index * m_inputStride + m_inputOffset;
  }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr Index srcCoeffOuterChipping(Index index) const {
