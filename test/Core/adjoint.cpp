...
TEST(AdjointTest, InnerProductBoundary) {
  // Inner product vectorization boundary tests (deterministic, outside g_repeat)
  inner_product_boundary_sizes<float>();
  inner_product_boundary_sizes<double>();
  inner_product_boundary_sizes<std::complex<float>>();
  inner_product_boundary_sizes<std::complex<double>>();
}

TEST(AdjointTest, TransposeInPlaceBoundary) {
  // transposeInPlace at vectorization boundaries (deterministic, outside g_repeat).
  transposeInPlace_boundary<float>();
  transposeInPlace_boundary<double>();
  transposeInPlace_boundary<std::complex<float>>();
}
