 ...
 private:
  ...
  PlainMatrix solve_impl(const MatrixBase<Rhs>& B, Index trunc, RealScalar lambda) const {
    ...
    internal::DeviceBuffer d_tmp(static_cast<size_t>(kk) * static_cast<size_t>(nrhs) * sizeof(Scalar));
    {
      ...
      auto* U = static_cast<const Scalar*>(d_U_.ptr);
      auto* VT = static_cast<const Scalar*>(d_VT_.ptr);
      auto* B_dev = static_cast<const Scalar*>(d_B.ptr);
      auto* tmp = static_cast<Scalar*>(d_tmp.ptr);

      if (!transposed_) {
        // U_stored^H * B: (m_×kk)^H × (m_×nrhs) → kk×nrhs.
        EIGEN_CUBLAS_CHECK(internal::cublasXgemm(cublas_, CUBLAS_OP_C, CUBLAS_OP_N, static_cast<int>(kk),
                                                 static_cast<int>(nrhs), static_cast<int>(m_), &scalars[0], U,
                                                 static_cast<int>(m_), B_dev, static_cast<int>(m_orig), &scalars[1],
                                                 tmp, static_cast<int>(kk)));
      } else {
        ...
        EIGEN_CUBLAS_CHECK(internal::cublasXgemm(cublas_, CUBLAS_OP_N, CUBLAS_OP_N, static_cast<int>(kk),
                                                 static_cast<int>(nrhs), static_cast<int>(m_orig), &scalars[0], VT,
                                                 static_cast<int>(vtrows_stored), B_dev, static_cast<int>(m_orig),
                                                 &scalars[1], tmp, static_cast<int>(kk)));
      }
    }
    ...
    PlainMatrix X(n_orig, nrhs);
    {
      ...
      if (!transposed_) {
        const Index vtrows = (options_ & ComputeFullV) ? n_ : k;
        EIGEN_CUBLAS_CHECK(internal::cublasXgemm(cublas_, CUBLAS_OP_C, CUBLAS_OP_N, static_cast<int>(n_orig),
                                                 static_cast<int>(nrhs), static_cast<int>(kk), &scalars[0],
                                                 static_cast<const Scalar*>(d_VT_.ptr), static_cast<int>(vtrows),
                                                 static_cast<const Scalar*>(d_tmp.ptr), static_cast<int>(kk),
                                                 &scalars[1], static_cast<Scalar*>(d_X.ptr),
                                                 static_cast<int>(n_orig)));
      } else {
        // U_stored is m_×ucols. V_orig = U_stored[:,:kk]. NoTrans × tmp.
        EIGEN_CUBLAS_CHECK(internal::cublasXgemm(cublas_, CUBLAS_OP_N, CUBLAS_OP_N, static_cast<int>(n_orig),
                                                 static_cast<int>(nrhs), static_cast<int>(kk), &scalars[0],
                                                 static_cast<const Scalar*>(d_U_.ptr), static_cast<int>(m_),
                                                 static_cast<const Scalar*>(d_tmp.ptr), static_cast<int>(kk),
                                                 &scalars[1], static_cast<Scalar*>(d_X.ptr),
                                                 static_cast<int>(n_orig)));
      }
