...
class EigenSolver {
 public:
  ...
  /** \brief Plain matrix type with the shape and storage options of #MatrixType; #MatrixType itself unless that
   * is a Ref<>. It is the type of the real matrices returned by pseudoEigenvectors() and pseudoEigenvalueMatrix(). */
   ...
   * destroyed; the results are stored in the decomposition object. Otherwise this constructor behaves like
 ...
 protected:
  ...
  // Holds the Schur form T and U of the last matrix; computing the eigenvectors overwrites T with the eigenvectors
  // of T and U with the pseudo-eigenvectors.
...
typename EigenSolver<MatrixType>::PlainMatrixType EigenSolver<MatrixType>::pseudoEigenvalueMatrix() const {
  ...
  PlainMatrixType matD = PlainMatrixType::Zero(n, n);
  Index i = 0;
  for (; i < n - 1; ++i) {
...
typename EigenSolver<MatrixType>::EigenvectorsType EigenSolver<MatrixType>::eigenvectors() const {
  ...
  eigen_assert(m_eigenvectorsOk && "The eigenvectors have not been computed together with the eigenvalues.");
  const RealScalar precision = RealScalar(2) * NumTraits<RealScalar>::epsilon();
  const PlainMatrixType& eivec = m_realSchur.m_matU;
  Index n = eivec.cols();
  ...
  for (Index j = 0; j < n; ++j) {
    if (internal::isMuchSmallerThan(numext::imag(m_eivalues.coeff(j)), numext::real(m_eivalues.coeff(j)), precision) ||
      ...
      matV.col(j) = eivec.col(j).template cast<ComplexScalar>();
      matV.col(j).normalize();
    } else {
      // we have a pair of complex eigen values
      for (Index i = 0; i < n; ++i) {
        matV.coeffRef(i, j) = ComplexScalar(eivec.coeff(i, j), eivec.coeff(i, j + 1));
        matV.coeffRef(i, j + 1) = ComplexScalar(eivec.coeff(i, j), -eivec.coeff(i, j + 1));
      }
      matV.col(j).normalize();
...
EigenSolver<MatrixType>& EigenSolver<MatrixType>::computeFromSchur(bool computeEigenvectors) {
  ...
  if (m_info == Success) {
    const MatrixType& matT = m_realSchur.m_matT;
    const Index n = matT.cols();

    // Compute eigenvalues from matT
    m_eivalues.resize(n);
    Index i = 0;
    while (i < n) {
      if (i == n - 1 || matT.coeff(i + 1, i) == Scalar(0)) {
        ...
        Scalar z;
        // Compute z = sqrt(abs(p * p + matT.coeff(i+1, i) * matT.coeff(i, i+1)));
        // without overflow
        {
...
void EigenSolver<MatrixType>::doComputeEigenvectors() {
  ...
  PlainMatrixType& eivec = m_realSchur.m_matU;
  ...
  for (Index n = size - 1; n >= 0; n--) {
    ...
    } else if (q < Scalar(0) && n > 0)  // Complex vector
    {
      ...
      if (abs(matT.coeff(n, n - 1)) > abs(matT.coeff(n - 1, n))) {
        ...
        ComplexScalar cc =
            ComplexScalar(Scalar(0), -matT.coeff(n - 1, n)) / ComplexScalar(matT.coeff(n - 1, n - 1) - p, q);
        matT.coeffRef(n - 1, n - 1) = numext::real(cc);
        matT.coeffRef(n - 1, n) = numext::imag(cc);
      }
      matT.coeffRef(n, n - 1) = Scalar(0);
      matT.coeffRef(n, n) = Scalar(1);
        ...
        if (m_eivalues.coeff(i).imag() < Scalar(0)) {
          ...
          if (m_eivalues.coeff(i).imag() == RealScalar(0)) {
            ...
            if (abs(x) > (abs(lastw) + abs(q))) {
              ...
              cc = ComplexScalar(-lastra - y * matT.coeff(i, n - 1), -lastsa - y * matT.coeff(i, n)) /
                   ComplexScalar(lastw, q);
              matT.coeffRef(i + 1, n - 1) = numext::real(cc);
              matT.coeffRef(i + 1, n) = numext::imag(cc);
            }
          }
  ...
  for (Index j = size - 1; j >= 0; j--) {
    m_tmp.noalias() = eivec.leftCols(j + 1) * matT.col(j).segment(0, j + 1);
    eivec.col(j) = m_tmp;
  }
}
