...
class BDCSVD : public SVDBase<BDCSVD<MatrixType_, Options_> > {
 ...
 public:
  ...
  constexpr BDCSVD(const MatrixBase<Derived>& matrix) : m_algoswap(16), m_numIters(0) {
   ...
   */
  template <typename Derived>
  EIGEN_DEPRECATED constexpr BDCSVD(const MatrixBase<Derived>& matrix, unsigned int computationOptions)
   ...
   */
  template <typename Derived>
  EIGEN_DEPRECATED constexpr BDCSVD& compute(const MatrixBase<Derived>& matrix, unsigned int computationOptions) {
    internal::check_svd_options_assertions<MatrixType, Options>(computationOptions, matrix.rows(), matrix.cols());
