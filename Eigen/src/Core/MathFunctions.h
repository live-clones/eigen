...
struct count_bits_impl {
  ...
  static EIGEN_DEVICE_FUNC inline constexpr int clz(BitsType bits) {
  ...
  static EIGEN_DEVICE_FUNC inline constexpr int ctz(BitsType bits) {
    int n = CHAR_BIT * sizeof(BitsType);
    int shift = n / 2;
    while (bits > 0 && shift > 0) {
      BitsType y = bits << shift;
...
EIGEN_DEVICE_FUNC inline constexpr int clz(BitsType bits) {
  return count_bits_impl<BitsType>::clz(bits);
}

// Count trailing zeros.
template <typename BitsType>
EIGEN_DEVICE_FUNC inline constexpr int ctz(BitsType bits) {
...
struct log_2_impl {
  ...
  static EIGEN_DEVICE_FUNC inline constexpr int run_ceil(const BitsType& x) {
    const int n = kTotalBits - clz(x);
    bool power_of_two = (x & (x - 1)) == 0;
    return x == 0 ? 0 : power_of_two ? (n - 1) : n;
  }
  static EIGEN_DEVICE_FUNC inline constexpr int run_floor(const BitsType& x) {
    const int n = kTotalBits - clz(x);
    return x == 0 ? 0 : n - 1;
  }
};
...
template <typename Scalar>
EIGEN_DEVICE_FUNC inline constexpr EIGEN_MATHFUNC_RETVAL(negate, Scalar) negate(const Scalar& x) {
