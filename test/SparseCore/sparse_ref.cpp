...
TEST(SparseRefTest, Basic) {
  for (int i = 0; i < g_repeat; i++) {
    check_const_correctness(SparseMatrix<float>());
    check_const_correctness(SparseMatrix<double, RowMajor>());
    call_ref();

    check_const_correctness(SparseVector<float>());
