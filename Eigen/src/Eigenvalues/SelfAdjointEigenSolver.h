...
EIGEN_DEVICE_FUNC SelfAdjointEigenSolver<MatrixType>& SelfAdjointEigenSolver<MatrixType>::compute(
  ...
  // and subsequent QR iteration. Since all entries are scaled uniformly, the tridiagonal
  // output is well-conditioned and no per-block scaling is needed in the QR iteration.
  ...
  while (end > 0) {
    ...
    if (PerBlockScaling) {
      ...
      if (start != scaled_start || end != scaled_end) {
        ...
        // Compute the norm and scale factor for the new block.
