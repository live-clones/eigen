...
1. **Provenance and attribution — no plagiarism, no license laundering.** Code contributed to Eigen must be original work or derived from publicly published material under a license compatible with Eigen's MPL-2.0. Do **not** copy code — verbatim, paraphrased, or "translated" to a different syntax — from incompatibly-licensed sources (proprietary code, NDA-encumbered material, prior-employer internal code, or anything whose license terms conflict with MPL-2.0 redistribution). Lightly rewording a copied implementation is still a derivative work. **Cite your references** when a published source informs an implementation — a LAPACK routine, a LAPACK Working Note (LAWN), an ACM TOMS or SIAM paper, Higham, Golub & van Loan, a textbook algorithm, a Boost component, a vendor application note — by naming it inline (author, year, identifier; a brief comment or Doxygen `\note` block suffices). The same discipline applies to AI-suggested code: if a generative model produces a non-trivial block, identify and cite its source; if you cannot, rewrite the routine from a known published reference (and cite that) or drop the suggestion. Ideas aren't copyrightable, but specific expressions of them are — when in doubt, learn from the source, write your own implementation, and credit it.
