...
class SparseVector : public SparseCompressedBase<SparseVector<Scalar_, Options_, StorageIndex_> > {
 ...
 public:
  ...
  constexpr const StorageIndex* outerIndexPtr() const { return 0; }
  constexpr StorageIndex* outerIndexPtr() { return 0; }
  constexpr const StorageIndex* innerNonZeroPtr() const { return 0; }
  constexpr StorageIndex* innerNonZeroPtr() { return 0; }
 ...
 public:
  ...
  constexpr void setZero() { m_data.clear(); }

  /** \returns the number of non zero coefficients */
  constexpr Index nonZeros() const { return m_data.size(); }
  ...
  constexpr Scalar& insertBack(Index i) {
  ...
  constexpr Scalar& insertBackUnordered(Index i) {
    m_data.append(Scalar(0), i);
    return m_data.value(m_data.size() - 1);
  }
  ...
  /**
   */
  constexpr void reserve(Index reserveSize) { m_data.reserve(reserveSize); }
  ...
  constexpr SparseVector() : m_size(0) { resize(0); }

  explicit constexpr SparseVector(Index size) : m_size(0) { resize(size); }

  constexpr SparseVector(Index rows, Index cols) : m_size(0) { resize(rows, cols); }

  template <typename OtherDerived>
  constexpr SparseVector(const SparseMatrixBase<OtherDerived>& other) : m_size(0) {
  ...
  constexpr SparseVector(const SparseVector& other) : Base(other), m_size(0) { *this = other.derived(); }

  /** Swaps the values of \c *this and \a other.
   * Overloaded for performance: this version performs a \em shallow swap by swapping pointers and attributes only.
   * \sa SparseMatrixBase::swap()
   */
  constexpr void swap(SparseVector& other) {
  ...
  constexpr void swap(SparseMatrix<Scalar, OtherOptions, StorageIndex>& other) {
  ...
  constexpr SparseVector& operator=(const SparseVector& other) {
    if (other.isRValue()) {
  ...
  template <typename OtherDerived>
  constexpr SparseVector& operator=(const SparseMatrixBase<OtherDerived>& other) {
  ...
  constexpr SparseVector(SparseVector&& other) : SparseVector() { this->swap(other); }

  template <typename OtherDerived>
  constexpr SparseVector(SparseCompressedBase<OtherDerived>&& other) : SparseVector() {
    *this = other.derived().markAsRValue();
  }

  constexpr SparseVector& operator=(SparseVector&& other) {
    this->swap(other);
    return *this;
  }

  template <typename OtherDerived>
  constexpr SparseVector& operator=(SparseCompressedBase<OtherDerived>&& other) {
    *this = other.derived().markAsRValue();
    return *this;
  }
  ...
  constexpr SparseVector& operator=(const SparseSparseProduct<Lhs, Rhs>& product) {
 ...
 public:
  ...
  EIGEN_DEPRECATED_WITH_REASON("Use .setZero() and .reserve() instead.") constexpr void startFill(Index reserve) {
    setZero();
    m_data.reserve(reserve);
  }

  /** \internal \deprecated use insertBack(Index,Index) */
  EIGEN_DEPRECATED_WITH_REASON("Use .insertBack() instead.") constexpr Scalar& fill(Index r, Index c) {
    eigen_assert(r == 0 || c == 0);
    return fill(IsColVector ? r : c);
  }

  /** \internal \deprecated use insertBack(Index) */
  EIGEN_DEPRECATED_WITH_REASON("Use .insertBack() instead.") constexpr Scalar& fill(Index i) {
    m_data.append(Scalar(0), i);
    return m_data.value(m_data.size() - 1);
  }

  /** \internal \deprecated use insert(Index,Index) */
  EIGEN_DEPRECATED_WITH_REASON("Use .insert() instead.") constexpr Scalar& fillrand(Index r, Index c) {
    eigen_assert(r == 0 || c == 0);
    return fillrand(IsColVector ? r : c);
  }

  /** \internal \deprecated use insert(Index) */
  EIGEN_DEPRECATED_WITH_REASON("Use .insert() instead.") constexpr Scalar& fillrand(Index i) { return insert(i); }

  /** \internal \deprecated use finalize() */
  EIGEN_DEPRECATED_WITH_REASON("Use .finalize() instead.") constexpr void endFill() {}

  // These two functions were here in the 3.1 release, so let's keep them in case some code rely on them.
  /** \internal \deprecated use data() */
  EIGEN_DEPRECATED_WITH_REASON("Use .data() instead.") constexpr Storage& _data() { return m_data; }
  /** \internal \deprecated use data() */
  EIGEN_DEPRECATED_WITH_REASON("Use .data() instead.") constexpr const Storage& _data() const { return m_data; }
