...
class SelfAdjointEigenSolver {
 public:
   ...
   *
   * \b Subsets. This is the natural companion to the #UseBisection eigenvalue path, which can resolve
   * an arbitrary contiguous subset of the spectrum (see EigenvalueRange): pass that subset as
   * \a eigenvalues and you get back exactly those \c m eigenvectors -- the result has one column per
   * supplied eigenvalue and costs one inverse-iteration solve each, so you never pay for vectors you did
   * not ask for. The eigenvectors are those of the tridiagonal \f$ T \f$ itself; recovering eigenvectors
   * of a dense matrix additionally requires the Householder back-transform performed by compute().
   *
