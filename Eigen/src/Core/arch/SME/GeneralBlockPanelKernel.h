 ...
 * SME constants
 *
 * On Apple M4 (armv9.2-a+sme2, SVL=512), the streaming vector length is
 * 512 bits = 16 × float32.  Each ZA tile for FP32 is 16×16.
 * There are 4 ZA tiles (ZA0.S–ZA3.S), so the full ZA storage is 16×64.
 *
 * Following the MpGEMM paper (arXiv:2512.21473), the optimal micro-kernel
 * shape for FP32 is  mr × nr = SVL32 × (4 × SVL32) = 16 × 64:
 *   – LHS panels are 16-wide  (one full SVE register load per depth step)
 *   – RHS super-panels are 64-wide (4 × 16-wide sub-panels interleaved)
 *   – The micro-kernel processes one 16×64 super-panel, using all
 *     4 ZA tiles for simultaneous accumulation
 *   – Each depth step performs 4 FMOPA instructions (one per ZA tile)
 *   – Data is loaded in groups of 4 Z registers for peak bandwidth
 *
 * The packing layout is designed so that an SME packer using LD1W/ST1W
 * or even multi-vector LDR/STR can be swapped in later without changing
 * the kernel, since the memory layout is identical to what sequential
 * scalar packing produces.
 *****************************************************************************/

// Streaming vector length in floats (= SVL / 32).
static constexpr int kSmeVlFloats = 16;
...
//   accumulating into all 4 ZA tiles simultaneously → 16×64 output tile.
...
/*****************************************************************************
 * gebp_traits specialization for SME  (float × float)
 *
 ...
 *   – gemm_pack_rhs receives nr = 64, creating 4×VL-wide RHS super-panels
...
class gebp_traits<float, float, false, false, Architecture::Target, GEBPPacketFull>
 ...
 public:
  ...
  enum {
    ...
    nr = kSmeNr4  // 64 — RHS super-panel width (4 × VL)
  };
};
 ...
 *   ...
 *   Last panel (tail, zero-padded to mr)
 *
 ...
 * Each depth step k writes nr contiguous floats (4 × VL).
...
static inline __attribute__((always_inline)) void sme_store_za_tile(Scalar* EIGEN_RESTRICT C, Index C_stride_row,
                                                                    Index C_stride_col, Scalar alpha, Index row_start,
                                                                    int pw, Index col_start, int cw,
                                                                    int tile_id) __arm_streaming __arm_inout("za") {
  ...
  if (C_stride_row == 1) {
    ...
    for (int ci = 0; ci < cw; ++ci) {
      ...
      switch (tile_id) {
        case 0:
          vres = svread_ver_za32_f32_m(svdup_f32(0.f), pg_m, 0, (uint32_t)ci);
          break;
        case 1:
          vres = svread_ver_za32_f32_m(svdup_f32(0.f), pg_m, 1, (uint32_t)ci);
          break;
        case 2:
          vres = svread_ver_za32_f32_m(svdup_f32(0.f), pg_m, 2, (uint32_t)ci);
          break;
        default:
          vres = svread_ver_za32_f32_m(svdup_f32(0.f), pg_m, 3, (uint32_t)ci);
          break;
      }
    ...
    for (int ri = 0; ri < pw; ++ri) {
      ...
      switch (tile_id) {
        case 0:
          vres = svread_hor_za32_f32_m(svdup_f32(0.f), pg_n, 0, (uint32_t)ri);
          break;
        case 1:
          vres = svread_hor_za32_f32_m(svdup_f32(0.f), pg_n, 1, (uint32_t)ri);
          break;
        case 2:
          vres = svread_hor_za32_f32_m(svdup_f32(0.f), pg_n, 2, (uint32_t)ri);
          break;
        default:
          vres = svread_hor_za32_f32_m(svdup_f32(0.f), pg_n, 3, (uint32_t)ri);
          break;
      }
...
/*****************************************************************************
 * sme_process_4tiles — Interleaved 4-tile micro-kernel (pw × 64 output)
 *
 ...
 *   [col0..col15 | col16..col31 | col32..col47 | col48..col63]  × depth
 * i.e., 64 contiguous floats per depth step — used by svld1_f32_x4.
 *****************************************************************************/

template <typename Scalar, typename Index>
static inline __attribute__((always_inline)) void sme_process_4tiles(
    Scalar* EIGEN_RESTRICT C, Index C_stride_row, Index C_stride_col, const Scalar* EIGEN_RESTRICT blA,
    const Scalar* EIGEN_RESTRICT blB, Index depth, Scalar alpha, Index row_start, int pw,
  ...
  sme_store_za_tile(C, C_stride_row, C_stride_col, alpha, row_start, pw, col_start + 0 * NR, NR, 0);
  sme_store_za_tile(C, C_stride_row, C_stride_col, alpha, row_start, pw, col_start + 1 * NR, NR, 1);
  sme_store_za_tile(C, C_stride_row, C_stride_col, alpha, row_start, pw, col_start + 2 * NR, NR, 2);
  sme_store_za_tile(C, C_stride_row, C_stride_col, alpha, row_start, pw, col_start + 3 * NR, NR, 3);
}

/*****************************************************************************
 * sme_process_microblock — Single-tile micro-kernel for tail columns.
 *
 * Processes one micro-block of size pw × cw × depth using a single ZA tile.
...
template <typename Scalar, typename Index>
static inline __attribute__((always_inline)) void sme_process_microblock(
    Scalar* EIGEN_RESTRICT C, Index C_stride_row, Index C_stride_col, const Scalar* EIGEN_RESTRICT blA,
    const Scalar* EIGEN_RESTRICT blB, int blB_stride, Index depth, Scalar alpha, Index row_start, int pw,
    Index col_start, int cw, int tile_id) __arm_streaming __arm_inout("za") {
  const svbool_t pg_m = svwhilelt_b32((uint32_t)0, (uint32_t)pw);
  const svbool_t pg_n = svwhilelt_b32((uint32_t)0, (uint32_t)cw);
  ...
  for (Index k = 0; k < depth; ++k) {
    ...
    switch (tile_id) {
      case 0:
        ...
        break;
      case 1:
        svmopa_za32_f32_m(1, pg_m, pg_n, va, vb);
        break;
      case 2:
        svmopa_za32_f32_m(2, pg_m, pg_n, va, vb);
        break;
      case 3:
        svmopa_za32_f32_m(3, pg_m, pg_n, va, vb);
        break;
    }
  }

  sme_store_za_tile(C, C_stride_row, C_stride_col, alpha, row_start, pw, col_start, cw, tile_id);
}
...
template <typename Scalar, typename Index>
static inline __attribute__((always_inline)) void sme_process_all_cols(
    Scalar* EIGEN_RESTRICT C, Index C_stride_row, Index C_stride_col, const Scalar* EIGEN_RESTRICT blA,
    const Scalar* EIGEN_RESTRICT blockB, Index depth, Index cols, Scalar alpha, Index strideB, Index offsetB,
  ...
  if (peeled_cols < cols) {
    ...
    int tile = 0;
    for (int t = 0; t < n_full_sub; ++t, ++tile) {
      // Each sub-group's data starts at offset t*NR within each k-step,
      // but the stride between k-steps is `tail` (the full tail width).
      sme_process_microblock(C, C_stride_row, C_stride_col, blA, blB_tail + t * NR,
                             tail,  // blB_stride
                             depth, alpha, row_start, pw, peeled_cols + t * NR, NR, tile);
    }

    if (sub_tail > 0) {
      sme_process_microblock(C, C_stride_row, C_stride_col, blA, blB_tail + n_full_sub * NR,
                             tail,  // blB_stride
                             depth, alpha, row_start, pw, peeled_cols + n_full_sub * NR, sub_tail, tile);
    }
  }
}
...
/*****************************************************************************
 * gebp_kernel specialization for SME  (float × float)
 *
