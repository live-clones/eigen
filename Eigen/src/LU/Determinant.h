...
struct determinant_impl {
  static inline constexpr typename traits<Derived>::Scalar run(const Derived& m) {
...
struct determinant_impl<Derived, 1> {
  EIGEN_DEVICE_FUNC static inline constexpr typename traits<Derived>::Scalar run(const Derived& m) {
    return m.coeff(0, 0);
  }
};
...
struct determinant_impl<Derived, 4> {
  ...
  EIGEN_DEVICE_FUNC static constexpr Scalar run(const Derived& m) {
