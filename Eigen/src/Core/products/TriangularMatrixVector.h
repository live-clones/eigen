...
EIGEN_STRONG_INLINE void trmv_correct_unit_diagonal(const Lhs& lhs, const Rhs& rhs, Dest& dest,
...
struct trmv_selector<Mode, ColMajor> {
  ...
  static void run(const Lhs& lhs, const Rhs& rhs, Dest& dest, const typename Dest::Scalar& alpha) {
    ...
    actual_dest.copy_back(dest, dest, actualDestPtr);
...
struct trmv_selector<Mode, RowMajor> {
  ...
  static void run(const Lhs& lhs, const Rhs& rhs, Dest& dest, const typename Dest::Scalar& alpha) {
    ...
    gemv_prepare_rhs<DirectlyUseRhs, AlignedMax>(actualRhs, actualRhsPtr);
