...
TEST(ArrayForMatrixTest, LpNormMatrix) {
  // lpNorm for matrices (not just vectors)
  for (int i = 0; i < g_repeat; i++) {
    lpNorm_matrix(Matrix2f());
    lpNorm_matrix(Matrix4d());
    lpNorm_matrix(
        MatrixXf(internal::random<int>(1, EIGEN_TEST_MAX_SIZE), internal::random<int>(1, EIGEN_TEST_MAX_SIZE)));
    lpNorm_matrix(MatrixXcd(internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2),
                            internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2)));
  }
}

TEST(ArrayForMatrixTest, LpNormMatrixEmpty) {
  // empty matrix
  lpNorm_matrix(MatrixXf(0, 0));
  lpNorm_matrix(MatrixXf(0, 3));
}
