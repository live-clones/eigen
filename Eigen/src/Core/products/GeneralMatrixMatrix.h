...
struct general_matrix_matrix_product<Index, LhsScalar, LhsStorageOrder, ConjugateLhs, RhsScalar, RhsStorageOrder,
  ...
  static void run(Index rows, Index cols, Index depth, const LhsScalar* lhs_, Index lhsStride, const RhsScalar* rhs_,
    ...
    // Direct vectorized path for small matrices: bypass packing overhead.
    // When the total working set fits in L1, direct SIMD computation is faster
    // than pack + GEBP because packing overhead dominates for small sizes.
    if (LhsStorageOrder == ColMajor && !ConjugateLhs && !ConjugateRhs && resIncr == 1) {
      std::ptrdiff_t l1, l2, l3;
      manage_caching_sizes(GetAction, &l1, &l2, &l3);
      const Index working_set = rows * depth * sizeof(LhsScalar) + depth * cols * sizeof(RhsScalar) +
                                rows * cols * sizeof(ResScalar);
      if (EIGEN_PREDICT_FALSE(working_set <= l1 / 2)) {
        typedef typename packet_traits<ResScalar>::type ResPacket;
        const Index PacketSize = packet_traits<ResScalar>::size;
        const Index peeled_rows = (rows / PacketSize) * PacketSize;

// RHS element (k,j) access macro: handles both ColMajor and RowMajor storage.
// RhsStorageOrder is a compile-time template parameter so the branch is optimized away.
#define EIGEN_SMGEMM_RHS(k, j) \
  ResScalar(rhs_[RhsStorageOrder == ColMajor ? ((k) + (j) * rhsStride) : ((k) * rhsStride + (j))])

        if (alpha == ResScalar(1)) {
          // Fast path: alpha == 1, accumulate directly into result
          Index j = 0;
          for (; j + 4 <= cols; j += 4) {
            for (Index i = 0; i < peeled_rows; i += PacketSize) {
              ResPacket c0 = ploadu<ResPacket>(res_ + i + (j + 0) * resStride);
              ResPacket c1 = ploadu<ResPacket>(res_ + i + (j + 1) * resStride);
              ResPacket c2 = ploadu<ResPacket>(res_ + i + (j + 2) * resStride);
              ResPacket c3 = ploadu<ResPacket>(res_ + i + (j + 3) * resStride);
              for (Index k = 0; k < depth; ++k) {
                ResPacket a = ploadu<ResPacket>(lhs_ + i + k * lhsStride);
                c0 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 0)), c0);
                c1 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 1)), c1);
                c2 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 2)), c2);
                c3 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 3)), c3);
              }
              pstoreu(res_ + i + (j + 0) * resStride, c0);
              pstoreu(res_ + i + (j + 1) * resStride, c1);
              pstoreu(res_ + i + (j + 2) * resStride, c2);
              pstoreu(res_ + i + (j + 3) * resStride, c3);
            }
            for (Index i = peeled_rows; i < rows; ++i) {
              for (Index jj = 0; jj < 4; ++jj) {
                ResScalar acc(0);
                for (Index k = 0; k < depth; ++k)
                  acc += ResScalar(lhs_[i + k * lhsStride]) * EIGEN_SMGEMM_RHS(k, j + jj);
                res_[i + (j + jj) * resStride] += acc;
              }
            }
          }
          for (; j < cols; ++j) {
            for (Index i = 0; i < peeled_rows; i += PacketSize) {
              ResPacket c0 = ploadu<ResPacket>(res_ + i + j * resStride);
              for (Index k = 0; k < depth; ++k) {
                ResPacket a = ploadu<ResPacket>(lhs_ + i + k * lhsStride);
                c0 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j)), c0);
              }
              pstoreu(res_ + i + j * resStride, c0);
            }
            for (Index i = peeled_rows; i < rows; ++i) {
              ResScalar acc(0);
              for (Index k = 0; k < depth; ++k)
                acc += ResScalar(lhs_[i + k * lhsStride]) * EIGEN_SMGEMM_RHS(k, j);
              res_[i + j * resStride] += acc;
            }
          }
        } else {
          // General path: alpha != 1, accumulate A*B separately then scale
          const ResPacket alphav = pset1<ResPacket>(alpha);
          Index j = 0;
          for (; j + 4 <= cols; j += 4) {
            for (Index i = 0; i < peeled_rows; i += PacketSize) {
              ResPacket acc0 = pset1<ResPacket>(ResScalar(0));
              ResPacket acc1 = pset1<ResPacket>(ResScalar(0));
              ResPacket acc2 = pset1<ResPacket>(ResScalar(0));
              ResPacket acc3 = pset1<ResPacket>(ResScalar(0));
              for (Index k = 0; k < depth; ++k) {
                ResPacket a = ploadu<ResPacket>(lhs_ + i + k * lhsStride);
                acc0 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 0)), acc0);
                acc1 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 1)), acc1);
                acc2 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 2)), acc2);
                acc3 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j + 3)), acc3);
              }
              ResPacket c0 = ploadu<ResPacket>(res_ + i + (j + 0) * resStride);
              ResPacket c1 = ploadu<ResPacket>(res_ + i + (j + 1) * resStride);
              ResPacket c2 = ploadu<ResPacket>(res_ + i + (j + 2) * resStride);
              ResPacket c3 = ploadu<ResPacket>(res_ + i + (j + 3) * resStride);
              pstoreu(res_ + i + (j + 0) * resStride, pmadd(alphav, acc0, c0));
              pstoreu(res_ + i + (j + 1) * resStride, pmadd(alphav, acc1, c1));
              pstoreu(res_ + i + (j + 2) * resStride, pmadd(alphav, acc2, c2));
              pstoreu(res_ + i + (j + 3) * resStride, pmadd(alphav, acc3, c3));
            }
            for (Index i = peeled_rows; i < rows; ++i) {
              for (Index jj = 0; jj < 4; ++jj) {
                ResScalar acc(0);
                for (Index k = 0; k < depth; ++k)
                  acc += ResScalar(lhs_[i + k * lhsStride]) * EIGEN_SMGEMM_RHS(k, j + jj);
                res_[i + (j + jj) * resStride] += alpha * acc;
              }
            }
          }
          for (; j < cols; ++j) {
            for (Index i = 0; i < peeled_rows; i += PacketSize) {
              ...
              for (Index k = 0; k < depth; ++k) {
                ResPacket a = ploadu<ResPacket>(lhs_ + i + k * lhsStride);
                acc0 = pmadd(a, pset1<ResPacket>(EIGEN_SMGEMM_RHS(k, j)), acc0);
              }
              ResPacket c0 = ploadu<ResPacket>(res_ + i + j * resStride);
              pstoreu(res_ + i + j * resStride, pmadd(alphav, acc0, c0));
            }
            for (Index i = peeled_rows; i < rows; ++i) {
              ResScalar acc(0);
              for (Index k = 0; k < depth; ++k)
                acc += ResScalar(lhs_[i + k * lhsStride]) * EIGEN_SMGEMM_RHS(k, j);
              res_[i + j * resStride] += alpha * acc;
            }
          }
        }
#undef EIGEN_SMGEMM_RHS
        return;
      }
    }
