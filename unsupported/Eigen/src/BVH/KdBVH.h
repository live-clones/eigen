...
struct get_boxes_helper {
  constexpr void operator()(const ObjectList &objects, BoxIter boxBegin, BoxIter boxEnd, VolumeList &outBoxes) const {
...
struct get_boxes_helper<ObjectList, VolumeList, int> {
  constexpr void operator()(const ObjectList &objects, int, int, VolumeList &outBoxes) const {
...
class KdBVH {
 public:
  ...
  constexpr Index getRootIndex() const { return (int)boxes.size() - 1; }
  ...
  constexpr const Volume &getVolume(Index index) const { return boxes[index]; }
 ...
 private:
  ...
  struct VectorComparator  // compares vectors, or more specifically, VIPairs along a particular dimension
  {
    ...
    constexpr bool operator()(const VIPair &v1, const VIPair &v2) const { return v1.first[dim] < v2.first[dim]; }
    int dim;
  };
