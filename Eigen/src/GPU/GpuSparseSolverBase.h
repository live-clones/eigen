...
class SparseSolverBase {
 public:
  ...
  DenseMatrix solve(const MatrixBase<Rhs>& B) const {
    ...
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&b_cudss, static_cast<int64_t>(n_), nrhs, static_cast<int64_t>(n_), d_b.get(),
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&x_cudss, static_cast<int64_t>(n_), nrhs, static_cast<int64_t>(n_), d_x.get(),
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
 ...
 private:
  ...
  void create_cudss_matrix() {
    ...
    EIGEN_CUDSS_CHECK(cudssMatrixCreateCsr(
        &d_A_cudss_, static_cast<int64_t>(n_), static_cast<int64_t>(n_), static_cast<int64_t>(nnz_), d_rowPtr_.get(),
        /*rowEnd=*/nullptr, d_colIdx_.get(), d_values_.get(), idx_type, val_type, mtype, mview, CUDSS_BASE_ZERO));
  }
