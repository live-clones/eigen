...
class DenseCoeffsBase<Derived, ReadOnlyAccessors> : public EigenBase<Derived> {
 public:
  ...
  // strided array, which means exactly that the underlying data of coefficients does exist in memory, which means exactly
  // that the coefficients is const-referencable, which means exactly that we can have coeff() return a const reference. 
  // For example, Map<const Matrix> have DirectAccessBit but not LvalueBit, so that Map<const Matrix>.coeff() does
  // points to a const Scalar& which exists in memory, while does not allow coeffRef() as it would not provide a lvalue. 
  // Notice that DirectAccessBit and LvalueBit are mutually orthogonal.
  ...
  typedef std::conditional_t<bool(internal::traits<Derived>::Flags & LvalueBit) || 
                             bool(internal::traits<Derived>::Flags & DirectAccessBit), const Scalar&,
