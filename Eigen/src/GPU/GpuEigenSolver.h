...
class SelfAdjointEigenSolver {
 ...
 private:
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXsyevd_bufferSize(ctx_.cusolver_, ctx_.params_.p, jobz, uplo,
                                                     static_cast<int64_t>(n_), dtype, d_A_.get(), lda_, rtype,
                                                     d_W_.get(), dtype, &dev_ws, &host_ws));

    ctx_.ensure_scratch(dev_ws);
    ctx_.h_workspace_.resize(host_ws);

    EIGEN_CUSOLVER_CHECK(cusolverDnXsyevd(ctx_.cusolver_, ctx_.params_.p, jobz, uplo, static_cast<int64_t>(n_), dtype,
                                          d_A_.get(), lda_, rtype, d_W_.get(), dtype, ctx_.scratch_workspace(), dev_ws,
