...
class UpperBidiagonalization {
 public:
  ...
  using RowVectorType = Matrix<Scalar, 1, ColsAtCompileTime>;
  using ColVectorType = Matrix<Scalar, RowsAtCompileTime, 1>;
  using BidiagonalType = BandMatrix<RealScalar, ColsAtCompileTime, ColsAtCompileTime, 1, 0, RowMajor>;
  using DiagVectorType = Matrix<Scalar, ColsAtCompileTime, 1>;
  using SuperDiagVectorType = Matrix<Scalar, ColsAtCompileTimeMinusOne, 1>;
  using HouseholderUSequenceType =
      HouseholderSequence<const MatrixType,
                          const internal::remove_all_t<typename Diagonal<const MatrixType, 0>::ConjugateReturnType>>;
  using HouseholderVSequenceType =
      HouseholderSequence<const internal::remove_all_t<typename MatrixType::ConjugateReturnType>,
   ...
   */
  constexpr UpperBidiagonalization() : m_bidiagonal(m_householder.cols(), m_householder.cols()) {}

  constexpr explicit UpperBidiagonalization(const MatrixType& matrix)
      : m_householder(matrix.rows(), matrix.cols()), m_bidiagonal(matrix.cols(), matrix.cols()) {
    compute(matrix);
  }

  constexpr UpperBidiagonalization(Index rows, Index cols) : m_householder(rows, cols), m_bidiagonal(cols, cols) {}

  constexpr UpperBidiagonalization& compute(const MatrixType& matrix);
  constexpr UpperBidiagonalization& computeUnblocked(const MatrixType& matrix);

  constexpr const MatrixType& householder() const { return m_householder; }
  constexpr const BidiagonalType& bidiagonal() const { return m_bidiagonal; }

  constexpr const HouseholderUSequenceType householderU() const {
    eigen_assert(m_isInitialized && "UpperBidiagonalization is not initialized.");
    return HouseholderUSequenceType(m_householder, m_householder.diagonal().conjugate());
  }

  constexpr const HouseholderVSequenceType householderV()  // const here gives nasty errors and i'm lazy
  {
    eigen_assert(m_isInitialized && "UpperBidiagonalization is not initialized.");
...
constexpr void upperbidiagonalization_inplace_unblocked(MatrixType& mat, typename MatrixType::RealScalar* diagonal,
  ...
  using TempType = Matrix<Scalar, Dynamic, 1, ColMajor, MatrixType::MaxRowsAtCompileTime, 1>;
  TempType tempVector;
  if (tempData == 0) {
 ...
 */
template <typename MatrixType>
constexpr void upperbidiagonalization_blocked_helper(
  ...
  constexpr int StorageOrder = (traits<MatrixType>::Flags & RowMajorBit) ? RowMajor : ColMajor;
  using ColInnerStride = InnerStride<StorageOrder == ColMajor ? 1 : Dynamic>;
  using RowInnerStride = InnerStride<StorageOrder == ColMajor ? Dynamic : 1>;
  using SubColumnType = Ref<Matrix<Scalar, Dynamic, 1>, 0, ColInnerStride>;
  using SubRowType = Ref<Matrix<Scalar, 1, Dynamic>, 0, RowInnerStride>;
  using SubMatType = Ref<Matrix<Scalar, Dynamic, Dynamic, StorageOrder>>;
...
constexpr void upperbidiagonalization_inplace_blocked(MatrixType& A, BidiagType& bidiagonal, Index maxBlockSize = 16,
                                                      typename MatrixType::Scalar* /*tempData*/ = 0) {
  using Scalar = typename MatrixType::Scalar;
...
template <typename MatrixType_>
constexpr UpperBidiagonalization<MatrixType_>& UpperBidiagonalization<MatrixType_>::computeUnblocked(
...
template <typename MatrixType_>
constexpr UpperBidiagonalization<MatrixType_>& UpperBidiagonalization<MatrixType_>::compute(const MatrixType_& matrix) {
  Index rows = matrix.rows();
  Index cols = matrix.cols();
