...
// =============================================================================
TEST(EvaluatorsTest, TransposeAndBasic) {
  ...
  VERIFY((w.array() >= -1).all() && (w.array() <= 1).all());
...
TEST(EvaluatorsTest, ProductExpressions) {
  Vector2d v = Vector2d::Random();
  RowVector2d w;
...
TEST(EvaluatorsTest, ProductSizes) {
...
TEST(EvaluatorsTest, Expressions) {
  Vector2d v = Vector2d::Random();
  RowVector2d w;
...
TEST(EvaluatorsTest, CompoundAssignment) {
...
TEST(EvaluatorsTest, Triangular) {
...
TEST(EvaluatorsTest, DiagonalAndCost) {
...
TEST(EvaluatorsTest, RestrictedPacketAssignment) {
