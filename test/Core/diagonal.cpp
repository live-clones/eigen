...
TEST(DiagonalTest, DiagonalOfProduct) {
  // Diagonal-of-product optimization (deterministic, outside g_repeat).
  diagonal_of_product<float>();
  diagonal_of_product<double>();
  diagonal_of_product<std::complex<float>>();
}

TEST(DiagonalTest, SelectBoundary) {
  // Select at vectorization boundaries (deterministic, outside g_repeat).
  select_boundary<float>();
  select_boundary<double>();
  select_boundary<int>();
}
