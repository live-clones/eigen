...
EIGEN_DECLARE_TEST(skew_symmetric_matrix3) {
  for (int i = 0; i < g_repeat; i++) {
    ...
    skewSymmetricMultiplication(MatrixXf(3, internal::random<int>(3, EIGEN_TEST_MAX_SIZE)));
    skewSymmetricMultiplication(MatrixXd(3, internal::random<int>(3, EIGEN_TEST_MAX_SIZE)));
    traceAndDet<float>();
    traceAndDet<double>();
    transpose<float>();
    transpose<double>();
    ...
    exponentialOrthogonality<double>();
    exponentialRotation<float>();
    exponentialRotation<double>();
  }
}
