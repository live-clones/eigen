...
*/
template <typename T>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE constexpr T generic_fast_erf_float(const T& x) {
...
template <typename T>
struct erf_impl {
  EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE constexpr T run(const T& x) { return generic_fast_erf_float(x); }
};
