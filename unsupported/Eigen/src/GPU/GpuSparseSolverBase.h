...
class SparseSolverBase {
 public:
  ...
  DenseMatrix solve(const MatrixBase<Rhs>& B) const {
    ...
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&b_cudss, n_, nrhs, n_, d_b.get(),
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&x_cudss, n_, nrhs, n_, d_x.get(),
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
 ...
 private:
  ...
  void create_cudss_matrix() {
    ...
    EIGEN_CUDSS_CHECK(cudssMatrixCreateCsr(
        &d_A_cudss_, n_, n_, nnz_, d_rowPtr_.get(),
  ...
  void create_placeholder_dense() {
    ...
    constexpr cudaDataType_t dtype = cuda_data_type<Scalar>::value;
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&d_x_cudss_, n_, 1, n_, nullptr,
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
    EIGEN_CUDSS_CHECK(cudssMatrixCreateDn(&d_b_cudss_, n_, 1, n_, nullptr,
                                          dtype, CUDSS_LAYOUT_COL_MAJOR));
  }
};
