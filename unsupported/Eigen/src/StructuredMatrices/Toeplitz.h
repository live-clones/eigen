...
class Toeplitz : public EigenBase<Toeplitz<Scalar_, Rows_, Cols_>> {
 public:
   ...
   * overflow even for finite generators: the DFT accumulates up to p addends) or
   * the right-hand side -- takes the direct O(mn) kernel: the transforms would
   * smear a single Inf/NaN into NaNs across the whole output, where the dense
   * product only propagates it through the dot products that touch it. */
