...
class QR {
 public:
  ...
  QR& compute(DeviceMatrix<Scalar>&& d_A) {
    ...
    if (transposed_) {
      ...
      d_qr_ = internal::DeviceBuffer::adopt(static_cast<void*>(d_A.release()), factorBytes(), solver_ctx_.stream_);
    }
 ...
 private:
  ...
  void allocate_factor_storage(size_t mat_bytes) { internal::ensure_sized(d_qr_, mat_bytes, solver_ctx_.stream_); }
  ...
  void factorize() {
    ...
    internal::ensure_sized(d_tau_, static_cast<size_t>(k()) * sizeof(Scalar), solver_ctx_.stream_);
  ...
  PlainMatrix solve_overdetermined_host(const PlainMatrix& rhs) const {
    ...
    internal::DeviceBuffer d_B(b_bytes, solver_ctx_.stream_);
    EIGEN_CUDA_RUNTIME_CHECK(
  ...
  DeviceMatrix<Scalar> solve_overdetermined_device(const DeviceMatrix<Scalar>& d_B) const {
    ...
    internal::DeviceBuffer d_work(b_bytes, solver_ctx_.stream_);
    EIGEN_CUDA_RUNTIME_CHECK(
  ...
  PlainMatrix solve_underdetermined_host(const PlainMatrix& rhs, Index nrhs) const {
    ...
    internal::DeviceBuffer d_X(x_bytes, solver_ctx_.stream_);
