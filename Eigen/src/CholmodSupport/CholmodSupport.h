...
template <typename Scalar_, int Options_, typename Index_, int Rows_, int Cols_, int MaxNZ_>
const cholmod_sparse viewAsCholmod(const SparseVector<Scalar_, Options_, Index_, Rows_, Cols_, MaxNZ_>& mat) {
  cholmod_sparse res =
      viewAsCholmod(Ref<SparseMatrix<Scalar_, Options_, Index_, Rows_, Cols_, MaxNZ_> >(mat.const_cast_derived()));
  return res;
}
...
template <typename Scalar_, int Options_, typename Index_, int Rows_, int Cols_, int MaxNZ_, unsigned int UpLo>
